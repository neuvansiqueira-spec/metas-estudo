(() => {
    ...
    return `
      <article class="aldus-dup-v266-card" data-v270-topic="${escapeHtml(profile?.id || "")}">
        ...
        <div class="aldus-dup-v266-pair-grid" style="grid-template-columns:1fr">
          <section><small>${escapeHtml(profileDiscipline(profile))}</small><h4>${escapeHtml([profileCode(profile), profileLabel(profile)].filter(Boolean).join(" — "))}</h4></section>
        </div>
    ...
    return `
      <article class="aldus-dup-v266-card" data-v270-pair="${escapeHtml(pair.key || pairKey(pair.left, pair.right))}">
        ...
        <div class="aldus-dup-v266-pair-grid">
          <section><small>${escapeHtml(profileDiscipline(pair.left))}</small><h4>${escapeHtml([profileCode(pair.left), profileLabel(pair.left)].filter(Boolean).join(" — "))}</h4></section>
          <span aria-hidden="true">↔</span>
          <section><small>${escapeHtml(profileDiscipline(pair.right))}</small><h4>${escapeHtml([profileCode(pair.right), profileLabel(pair.right)].filter(Boolean).join(" — "))}</h4></section>
        </div>
    ...
    for (const left of selected) {
      for (const right of index.profiles) {
        ...
        if (!pair && (prefix || semantic)) {
          pair = { key, left, right, classification: "relation", confidence: prefix ? 72 : 64, reasons: [relationReason(left, right)] };
        }
  ...
  function renderSearch(root, query) {
    ...
    let html = customHeading("Busca no edital", `“${query}” — primeiro são localizados os temas; depois são mostrados somente os vínculos desses temas.`, result.direct.length);
    ...
    if (result.direct.length > directShown.length) html += `<p class="aldus-dup-v266-empty">Há mais ${result.direct.length - directShown.length} tema(s) correspondente(s). Refine a pesquisa.</p>`;
    ...
    html += pairShown.length ? pairShown.map(renderPair).join("") : '<p class="aldus-dup-v266-empty">O tema existe no edital, mas não possui vínculo diagnóstico com outra meta.</p>';
  ...
  function getAllRelations(targetState, diagnosticsApi) {
    ...
    (report?.pairs || []).forEach((pair) => {
      ...
      rows.set(pair.key || pairKey(pair.left, pair.right), pair);
    });
    ...
    for (let i = 0; i < index.profiles.length; i += 1) {
      for (let j = i + 1; j < index.profiles.length; j += 1) {
        ...
        rows.set(key, { key, left, right, classification: "relation", confidence: prefix ? 72 : 64, reasons: [relationReason(left, right)] });
      }
    }

    const sorted = [...rows.values()].sort((a, b) => (b.confidence - a.confidence) || profileLabel(a.left).localeCompare(profileLabel(b.left), "pt-BR"));
    relationCache = { signature: index.signature, rows: sorted };
    return sorted;
  }
  ...
  function renderRelations(root) {
    ...
    container.innerHTML = customHeading("Relações e sobreposições", "Preparando esta categoria…", 0);
    const all = getAllRelations(currentState, diagnosticsApi);
    const shown = all.slice(0, MAX_RELATION_RESULTS);
    container.innerHTML = customHeading("Relações e sobreposições", "Categoria informativa. Use Localizar para encontrar um tema específico sem recalcular todos os pares a cada tecla.", all.length)
