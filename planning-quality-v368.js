...
(() => {
  ...
  const repairStartDate = () => `${currentDate().slice(0, 7)}-01`;
  ...
  function bindGoalToItem(goal, item) {
    ...
    goal.planningQualityV368 = VERSION;
  ...
  function ensurePieceCatalog(targetState) {
    ...
    PIECES.forEach((definition) => {
      ...
      targetState.syllabusItems.push({
        ...
        supplementalDelegatePieceV368: true,
  ...
  function bindGoalToPiece(goal, definition, targetState) {
    ...
    goal.planningPieceRotationV368 = VERSION;
    return changed;
  }
  ...
  function repairPieceRotation(targetState) {
    ...
    indexed
      .filter(({ goal }) => looksLikePiece(goal) && goalDate(goal) && goalDate(goal) < startDate)
    ...
    indexed.forEach(({ goal, index }) => {
      ...
      if (!date || date < startDate || !looksLikePiece(goal)) return;
  ...
  function repairDidacticSchedule(targetState) {
    ...
    targetState.dailyGoals.forEach((goal, index) => {
      if (goalDate(goal) < startDate || !isAutomaticNewStudy(goal)) return;
  ...
  function run(reason = "manual", options = {}) {
    ...
    if (changed && options.persist !== false) persist(`planning-quality-v368:${reason}`, options);
