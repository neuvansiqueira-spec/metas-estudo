(() => {
  const VERSION = "20260721-prompt-lei-modelo-v120";
  const MIGRATION_ID = "factoryLeiModeloTopificadoV2";
  const V119_OFFICIAL_HASH = "58c97fda";
...
ENTREGUE O WORD COMPLETO E O LINK PARA DOWNLOAD. NÃO ENTREGUE APENAS O CONTEÚDO NO CHAT, SALVO PEDIDO EXPRESSO.`;
  ...
  const applyPrompt = () => {
    ...
    try {
      ...
      if (typeof state === "object") {
        ...
        const isPreviousOfficial = promptHash(current) === V119_OFFICIAL_HASH;
        if (!current || current === FALLBACK || isPreviousOfficial) {
      ...
      console.error("[Aldus] Falha ao aplicar o prompt Lei v120.", error);
    }
  };
  ...
  try {
    ...
    console.error("[Aldus] Falha ao proteger a geração do prompt Lei v120.", error);
  }
  ...
  window.addEventListener("load", () => {
    if ("serviceWorker" in navigator) {
      navigator.serviceWorker.register(`service-worker-v120.js?v=${encodeURIComponent(VERSION)}`, { updateViaCache: "none" })
