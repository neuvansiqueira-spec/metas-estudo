...
const ALDUS_V204_NAV_MODULE = "./side-nav-hover-collapse-v204.js";
...
const ALDUS_V204_NAV_MARKER = "/* Aldus runtime source: side-nav-hover-collapse-v204.js */";
...
async function aldusV204ApplicationResponse(baseResponse) {
  ...
  try {
    const [moduleResponse, visualResponse, navResponse] = await Promise.all([
      ...
      fetch(new URL(ALDUS_V204_NAV_MODULE, self.registration.scope), { cache: "no-store" })
    ]);
    ...
    let patchedSource = source.replaceAll(ALDUS_V199_VERSION, ALDUS_V204_VERSION);
    ...
    if (!patchedSource.includes(ALDUS_V204_NAV_MARKER)) {
      patchedSource = `${patchedSource.trim()}\n\n${ALDUS_V204_NAV_MARKER}\n${navSource.trim()}\n`;
    }
    const headers = new Headers(baseResponse.headers);
    for (const name of ["content-length", "content-encoding", "etag", "last-modified"]) headers.delete(name);
    headers.set("content-type", "text/javascript; charset=utf-8");
    headers.set("x-aldus-runtime-patch", ALDUS_V204_VERSION);
    ...
    console.warn("[Aldus V204] Não foi possível ativar o menu lateral por aproximação; a V199 será mantida.", error);
    return baseResponse;
  }
}
...
aldusNativeAddEventListener("fetch", (event) => {
  ...
  event.respondWith(isApplication ? baseResponsePromise.then(aldusV204ApplicationResponse) : baseResponsePromise);
});
