(() => {
  ...
  function disciplineTokens(profile) {
    const stop = new Set(["direito", "e", "gestao", "publica", "publico", "legislacao", "especial"]);
  ...
  function relationReason(left, right) {
    ...
    if (administrativeControlRelation(left, right)) return "Tema amplo de controle da Administração Pública relacionado a modalidade específica de controle.";
    return "Relação temática informativa identificada pelo diagnóstico.";
  }

  function renderProfile(profile) {
    const title = [profileCode(profile), profileLabel(profile)].filter(Boolean).join(" — ");
    return `
      <article class="aldus-dup-v266-card aldus-dup-v271-card" data-v271-topic="${escapeHtml(profile?.id || "")}">
        ...
        <div class="aldus-dup-v266-pair-grid aldus-dup-v271-single">
          <section><small>${escapeHtml(profileDiscipline(profile))}</small><h4>${escapeHtml(title || "Tema sem título")}</h4></section>
        </div>
  ...
  function renderPair(pair) {
    ...
    const leftTitle = [profileCode(pair.left), profileLabel(pair.left)].filter(Boolean).join(" — ");
    const rightTitle = [profileCode(pair.right), profileLabel(pair.right)].filter(Boolean).join(" — ");
    return `
      <article class="aldus-dup-v266-card aldus-dup-v271-card" data-v271-pair="${escapeHtml(pair.key || pairKey(pair.left, pair.right))}">
        ...
        <div class="aldus-dup-v266-pair-grid">
          <section><small>${escapeHtml(profileDiscipline(pair.left))}</small><h4>${escapeHtml(leftTitle || "Tema sem título")}</h4></section>
          <span aria-hidden="true">↔</span>
          <section><small>${escapeHtml(profileDiscipline(pair.right))}</small><h4>${escapeHtml(rightTitle || "Tema sem título")}</h4></section>
        </div>
        ${reasons ? `<ul class="aldus-dup-v266-reasons">${reasons}</ul>` : ""}
        <p class="aldus-dup-v266-warning">Este vínculo foi limitado à mesma disciplina para impedir falsos positivos por numeração coincidente entre matérias diferentes.</p>
      </article>`;
  }
  ...
  function renderSearch(root, query) {
    ...
    let html = customHeading("Busca no edital", `“${query}” — localização direta por tema, disciplina ou código.`, result.direct.length);
    if (!result.direct.length) {
      container.innerHTML = html + '<p class="aldus-dup-v266-empty">Nenhum tema do edital corresponde à busca.</p>';
      return;
    }

    html += result.selected.map(renderProfile).join("");
    if (result.direct.length > result.selected.length) html += `<p class="aldus-dup-v266-empty">Há mais ${result.direct.length - result.selected.length} tema(s) correspondente(s). Refine a pesquisa.</p>`;
    html += customHeading("Vínculos do tema localizado", "Somente vínculos disciplinarmente compatíveis são mostrados.", result.pairs.length);
    const pairShown = result.pairs.slice(0, MAX_PAIR_RESULTS);
    html += pairShown.length ? pairShown.map(renderPair).join("") : '<p class="aldus-dup-v266-empty">O tema existe no edital, mas não possui vínculo diagnóstico válido com outra meta da mesma disciplina.</p>';
    if (result.pairs.length > pairShown.length) html += `<p class="aldus-dup-v266-empty">Exibindo ${pairShown.length} de ${result.pairs.length} vínculos. Refine a pesquisa.</p>`;
  ...
  function renderRelationsPrompt(root) {
    ...
    container.innerHTML = customHeading(
      ...
      "Digite um tema em Localizar. A busca calcula apenas os vínculos do tema encontrado e ignora coincidências numéricas entre disciplinas diferentes.",
      0
    );
  }

  function install(root) {
    if (!root || root.dataset.v271SearchInstalled) return;
    root.dataset.v271SearchInstalled = "true";
    root.classList.add("aldus-dup-v266", "aldus-dup-v271");

    const filter = root.querySelector("[data-dup-filter]");
    const search = root.querySelector("[data-v261-search]");
    const list = root.querySelector("[data-dup-list]");
    if (!filter || !search || !list) {
      root.dataset.v271SearchInstalled = "";
    ...
    root.addEventListener("input", (event) => {
      ...
      const container = setCustomMode(list, true);
      container.innerHTML = customHeading("Busca no edital", "Localizando o tema e vínculos válidos na mesma disciplina…", 0);
  ...
  const API = Object.freeze({ VERSION, normalize, disciplineCompatible, codePrefixRelation, directTopicSearch });
  globalThis.__aldusDuplicateSearchV271 = API;
