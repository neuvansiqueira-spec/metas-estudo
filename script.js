...
function qbTroubleQuestionIds() { return new Set((state.questionBankSessions || []).flatMap((s) => s.items || []).filter((i) => i.status === "errado" || i.status === "branco" || !i.marcado || i.marcado === "B").map((i) => i.id)); }
function qbScopeLabel() { return { all:"Banco completo", syllabus:"Somente assuntos do edital", week:"Assuntos da semana", weak:"Assuntos fracos", trouble:"Erradas/brancas", unseen:"Não estudados" }[elements.qbTrainingScope?.value || "all"] || "Banco completo"; }
function qbScopedBank() { const scope = elements.qbTrainingScope?.value || "all", bank = state.questionBank || []; if (scope === "all") return bank; if (scope === "trouble") { const ids = qbTroubleQuestionIds(); return bank.filter((q) => ids.has(q.id)); } const items = scope === "week" ? qbWeekSyllabusItems() : scope === "weak" ? qbWeakSyllabusItems() : scope === "unseen" ? qbUnstudiedSyllabusItems() : qbActiveSyllabusItems(); return bank.filter((q) => qbQuestionMatchesAnySyllabusItem(q, items)); }
...
function qbFilteredQuestions() { const search = canonical(elements.qbFilterSearch?.value || ""); return qbScopedBank().filter((q)=> (!elements.qbFilterDiscipline?.value || q.disciplina === elements.qbFilterDiscipline.value) && (!elements.qbFilterSubject?.value || q.assunto === elements.qbFilterSubject.value) && (!elements.qbFilterTheme?.value || q.tema === elements.qbFilterTheme.value) && (!elements.qbFilterBoard?.value || q.banca === elements.qbFilterBoard.value) && (!elements.qbFilterYear?.value || String(q.ano) === elements.qbFilterYear.value) && (!search || canonical([q.enunciado,q.disciplina,q.assunto,q.tema,q.banca,q.ano,q.referencia,q.orgao,q.cargo].join(" ")).includes(search))); }
...
function qbCascadeBase(fields = {}) { const discipline = fields.discipline ?? elements.qbFilterDiscipline?.value ?? "", subject = fields.subject ?? elements.qbFilterSubject?.value ?? "", theme = fields.theme ?? elements.qbFilterTheme?.value ?? "", board = fields.board ?? elements.qbFilterBoard?.value ?? ""; return qbScopedBank().filter((q) => (!discipline || q.disciplina === discipline) && (!subject || q.assunto === subject) && (!theme || q.tema === theme) && (!board || q.banca === board)); }
function qbRenderCascadingFilters() { const bank = qbScopedBank(); qbFillSelect(elements.qbFilterDiscipline, qbUnique(bank.map(q=>q.disciplina)), "Todas"); const discipline = elements.qbFilterDiscipline?.value || ""; qbFillSelect(elements.qbFilterSubject, qbUnique(qbCascadeBase({ discipline, subject:"", theme:"", board:"" }).map(q=>q.assunto)), "Todos"); const subject = elements.qbFilterSubject?.value || ""; qbFillSelect(elements.qbFilterTheme, qbUnique(qbCascadeBase({ discipline, subject, theme:"", board:"" }).map(q=>q.tema)), "Todos"); const theme = elements.qbFilterTheme?.value || ""; qbFillSelect(elements.qbFilterBoard, qbUnique(qbCascadeBase({ discipline, subject, theme, board:"" }).map(q=>q.banca)), "Todas"); const board = elements.qbFilterBoard?.value || ""; qbFillSelect(elements.qbFilterYear, qbUnique(qbCascadeBase({ discipline, subject, theme, board }).map(q=>q.ano)), "Todos"); }
function renderQuestionBank() { if (!elements.qbStats) return; normalizeStoredQuestionBank(); const bank = state.questionBank || [], scoped = qbScopedBank(), sessions = state.questionBankSessions || [], last = sessions[0]; qbRenderCascadingFilters(); renderQbSyllabusPackages(); const filteredTotal = qbFilteredQuestions().length; elements.qbStats.innerHTML = [["Questões no banco", bank.length], ["Questões filtradas", filteredTotal], ["Disciplinas disponíveis", qbUnique(scoped.map(q=>q.disciplina)).length], ["Assuntos disponíveis", qbUnique(scoped.map(q=>q.assunto)).length], ["Questões com gabarito", scoped.filter(qbHasKey).length], ["Assuntos do edital sem questões", qbMissingSyllabusWithoutQuestions()], ["Treinos realizados", sessions.length], ["Último treino", last ? new Date(last.createdAt).toLocaleString("pt-BR") : "Nenhum"]].map(([a,b])=>`<article class="stat-card"><span>${a}</span><strong>${escapeHTML(b)}</strong></article>`).join(""); }
...
function qbStart(items = qbFilteredQuestions()) { if (!items.length) return alert("Nenhuma questão encontrada com os filtros atuais."); if (elements.qbShuffleTraining?.checked) items = qbShuffle(items); items = items.slice(0, Math.max(1, Number(elements.qbTrainingLimit?.value) || items.length)); questionBankTraining = { id:createId(), createdAt:new Date().toISOString(), index:0, items, answers:{} }; elements.qbTrainingPanel.hidden = false; elements.qbResultPanel.hidden = true; qbRenderQuestion(); }
...
function qbPreview() { qbRenderCascadingFilters(); const list=qbFilteredQuestions(); const discipline = elements.qbFilterDiscipline?.value || "todas as disciplinas"; if (elements.qbMessage) elements.qbMessage.textContent = `Escopo: ${qbScopeLabel()} — ${discipline}: ${list.length} questões encontradas.`; if (elements.qbStats) renderQuestionBank(); elements.qbFilteredPreview.innerHTML = list.length ? list.slice(0,20).map((q,i)=>`<article class="question-bank-item"><strong>${i+1}. ${escapeHTML(q.disciplina)} — ${escapeHTML(q.assunto)}</strong><p>${escapeHTML(q.enunciado)}</p><div class="item-meta">${escapeHTML(q.banca||"-")} • ${escapeHTML(q.ano||"-")}</div></article>`).join("") : "Nenhuma questão encontrada."; }
...
[elements.qbTrainingScope,elements.qbFilterDiscipline,elements.qbFilterSubject,elements.qbFilterTheme,elements.qbFilterBoard,elements.qbFilterYear].filter(Boolean).forEach((el)=>el.addEventListener("change", qbPreview));
