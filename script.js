...
state.questionLogs ||= []; state.questionBank ||= []; state.questionBankSessions ||= []; state.questionErrorNotebook ||= [];
state.smartReviews ||= [];
...
function replaceState(nextState) { Object.keys(state).forEach((key) => delete state[key]); Object.assign(state, { ...cloneData(defaultState), ...(nextState || {}) }); state.edital = { ...defaultState.edital, ...(state.edital || {}) }; state.syllabusItems ||= []; state.schedulableSettings ||= {}; state.dailyGoals ||= []; state.questionLogs ||= []; state.questionBank ||= []; state.questionBankSessions ||= []; state.questionErrorNotebook ||= [];
...
function qbNotebookEntryFromAnswer(q) { const reason = qbErrorReason(q); if (!reason) return null; return { id:q.id, disciplina:q.disciplina, assunto:q.assunto, banca:q.banca||"", cargo:q.cargo||"", enunciado:q.enunciado, respostaMarcada:q.marcado||"", gabaritoCorreto:q.gabarito||"", justificativa:qbExplanationText(q), motivoErro:reason, dataRegistro:new Date().toISOString(), status:"pendente", quantidadeErros:1 }; }
function qbSaveNotebookItems(items) { state.questionErrorNotebook ||= []; items.forEach((q) => { const entry = qbNotebookEntryFromAnswer(q); if (!entry) return; const existing = state.questionErrorNotebook.find((item) => item.id === entry.id); if (existing) Object.assign(existing, entry, { status: existing.status || "pendente", quantidadeErros: (Number(existing.quantidadeErros) || 1) + 1 }); else state.questionErrorNotebook.unshift(entry); }); }
function qbFilteredNotebook() { const list = state.questionErrorNotebook || []; return list.filter((item) => (!elements.qbErrorFilterDiscipline?.value || item.disciplina === elements.qbErrorFilterDiscipline.value) && (!elements.qbErrorFilterSubject?.value || item.assunto === elements.qbErrorFilterSubject.value) && (!elements.qbErrorFilterStatus?.value || item.status === elements.qbErrorFilterStatus.value) && (!elements.qbErrorFilterReason?.value || item.motivoErro === elements.qbErrorFilterReason.value)); }
function qbNotebookStats() { const list = state.questionErrorNotebook || []; const by = (field) => Object.entries(list.reduce((a,i)=>{ const k=i[field]||"Sem informação"; a[k]=(a[k]||0)+(Number(i.quantidadeErros)||1); return a; },{})).sort((a,b)=>b[1]-a[1]).slice(0,3).map(([k,v])=>`${k} (${v})`).join(", ") || "-"; return { total:list.reduce((s,i)=>s+(Number(i.quantidadeErros)||1),0), disciplinas:by("disciplina"), assuntos:by("assunto"), dominadas:list.filter(i=>i.status==="dominado").length, pendentes:list.filter(i=>i.status==="pendente").length }; }
...
function qbRenderErrorNotebook() { if (!elements.qbErrorStats) return; qbRenderNotebookFilters(); const stats=qbNotebookStats(), list=qbFilteredNotebook(); elements.qbErrorStats.innerHTML = [["Total de erros",stats.total],["Disciplinas com mais erros",stats.disciplinas],["Assuntos com mais erros",stats.assuntos],["Questões dominadas",stats.dominadas],["Questões pendentes",stats.pendentes]].map(([a,b])=>`<article class="stat-card"><span>${a}</span><strong>${escapeHTML(b)}</strong></article>`).join(""); elements.qbErrorNotebookList.innerHTML = list.length ? list.map((item)=>`<article class="question-bank-item"><header><h4>${escapeHTML(item.disciplina)} — ${escapeHTML(item.assunto)}</h4><span class="badge ${item.status==='dominado'?'success':item.status==='revisado'?'neutral':'warn'}">${escapeHTML(item.status)}</span></header><div class="card-meta-grid"><span>Banca: ${escapeHTML(item.banca||'-')}</span><span>Cargo: ${escapeHTML(item.cargo||'-')}</span><span>Motivo: ${escapeHTML(qbErrorReasonLabel(item.motivoErro))}</span><span>Erros nesta questão: ${Number(item.quantidadeErros)||1}</span><span>Resposta: ${escapeHTML(item.respostaMarcada||'-')}</span><span>Gabarito: ${escapeHTML(item.gabaritoCorreto||'-')}</span><span>Registrado em: ${new Date(item.dataRegistro).toLocaleString('pt-BR')}</span></div><p>${escapeHTML(item.enunciado)}</p><p><strong>Justificativa:</strong> ${escapeHTML(item.justificativa||'Sem justificativa cadastrada')}</p><div class="actions"><button class="secondary-button" data-qb-error-status="revisado" data-qb-error-id="${escapeHTML(item.id)}" type="button">Marcar revisada</button><button data-qb-error-status="dominado" data-qb-error-id="${escapeHTML(item.id)}" type="button">Marcar dominada</button></div></article>`).join("") : "Nenhum registro no caderno de erros."; }
function qbSetNotebookStatus(id, status) { const item=(state.questionErrorNotebook||[]).find(i=>i.id===id); if (!item) return; item.status=status; item.dataRegistro=new Date().toISOString(); saveData(); qbRenderErrorNotebook(); }
function qbStartNotebookTraining() { const ids = new Set(qbFilteredNotebook().filter(i=>i.status!=="dominado").map(i=>i.id)); const items=(state.questionBank||[]).filter(q=>ids.has(q.id)); if (!items.length) return alert("Nenhuma questão pendente/revisada do caderno corresponde ao banco atual."); qbStart(items); }
...
function renderQuestionBank() { if (!elements.qbStats) return; normalizeStoredQuestionBank(); const bank = state.questionBank || [], scoped = qbScopedBank(), sessions = state.questionBankSessions || [], last = sessions[0]; qbRenderCascadingFilters(); renderQbSyllabusPackages(); renderQbDiagnostics(); renderQbSyllabusVerticalized(); renderQbAccordionSummaries(); qbRenderErrorNotebook(); const filteredTotal = qbFilteredQuestions().length; const lastPerformance = last?.hasAnyKey ? `${last.summary.correct}/${last.summary.total} • líquido ${last.summary.net}` : (last ? `${last.summary.total} questão(ões), sem gabarito` : "Nenhum"); const manage = document.getElementById("qbManageBank"); if (manage && bank.length && !manage.dataset.touched) manage.open = false; elements.qbStats.innerHTML = [["Questões no banco", bank.length], ["Questões filtradas", filteredTotal], ["Disciplinas", qbUnique(scoped.map(q=>q.disciplina)).length], ["Assuntos", qbUnique(scoped.map(q=>q.assunto)).length], ["Questões com gabarito", scoped.filter(qbHasKey).length], ["Treinos realizados", sessions.length], ["Último desempenho", lastPerformance]].map(([a,b])=>`<article class="stat-card"><span>${a}</span><strong>${escapeHTML(b)}</strong></article>`).join(""); }
...
function qbPreview() { qbPreviewVisible = true; if (elements.qbPreviewSection) elements.qbPreviewSection.hidden = false; qbRenderCascadingFilters(); const list=qbFilteredQuestions(); const discipline = elements.qbFilterDiscipline?.value || "todas as disciplinas"; const zeroMessage = qbSelectedZeroDisciplineMessage(); if (elements.qbMessage) elements.qbMessage.textContent = zeroMessage || `Escopo: ${qbScopeLabel()} — ${discipline}: ${list.length} questões encontradas.`; if (elements.qbStats) renderQuestionBank(); const hidden=Math.max(0,list.length-20); elements.qbFilteredPreview.innerHTML = list.length ? `<p class="item-meta">${list.length} questão(ões) encontrada(s). Exibindo ${Math.min(20,list.length)}.</p>` + list.slice(0,20).map((q,i)=>`<article class="question-bank-item qb-preview-item"><strong>${i+1}. ${escapeHTML(q.disciplina)} — ${escapeHTML(q.assunto)}</strong><div class="item-meta">Tema: ${escapeHTML(q.tema||"-")} • Banca: ${escapeHTML(q.banca||"-")} • Ano: ${escapeHTML(q.ano||"-")}</div><p>${escapeHTML(q.enunciado)}</p></article>`).join("") + (hidden ? `<details><summary>Ver mais ${hidden} item(ns)</summary>${list.slice(20,50).map((q,i)=>`<article class="question-bank-item qb-preview-item"><strong>${i+21}. ${escapeHTML(q.disciplina)} — ${escapeHTML(q.assunto)}</strong><p>${escapeHTML(q.enunciado)}</p></article>`).join("")}</details>` : "") : escapeHTML(qbNoResultsMessage()); }

[elements.qbErrorFilterDiscipline, elements.qbErrorFilterSubject, elements.qbErrorFilterStatus, elements.qbErrorFilterReason].forEach((el) => el?.addEventListener("change", qbRenderErrorNotebook));
elements.qbErrorNotebookList?.addEventListener("click", (event) => { const btn=event.target.closest("[data-qb-error-status]"); if (btn) qbSetNotebookStatus(btn.dataset.qbErrorId, btn.dataset.qbErrorStatus); });
elements.qbStartErrorNotebook?.addEventListener("click", qbStartNotebookTraining);
...
function qbRefreshAfterFilterChange() { qbRenderCascadingFilters(); renderQuestionBank(); qbPreviewVisible = false; if (elements.qbPreviewSection) elements.qbPreviewSection.hidden = true; if (elements.qbFilteredPreview) elements.qbFilteredPreview.innerHTML = "Clique em “Pré-visualizar” para listar as questões encontradas."; }
...
elements.qbFilterSearch?.addEventListener("input", qbRefreshAfterFilterChange);
