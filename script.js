...
function qbRenderErrorNotebook() { if (!elements.qbErrorStats) return; qbRenderNotebookFilters(); const stats=qbNotebookStats(), all=qbFilteredNotebook(), list=all.slice(0, QB_RENDER_LIMIT), hidden=Math.max(0, all.length - list.length); elements.qbErrorStats.innerHTML = [
  ...
  ].map(([label, value, title]) => `<article class="stat-card"><span>${label}</span><strong title="${escapeHTML(title)}">${escapeHTML(value)}</strong></article>`).join(""); elements.qbErrorNotebookList.innerHTML = all.length ? `<p class="item-meta">Exibindo ${list.length} de ${all.length} registro(s).${hidden ? " Ajuste os filtros para refinar." : ""}</p>` + list.map((item)=>`<article class="question-bank-item qb-error-notebook-row"><header><h4>${escapeHTML(item.disciplina)} — ${escapeHTML(item.assunto)}</h4><span class="badge ${item.status==='dominado'?'success':item.status==='revisado'?'neutral':'warn'}">${escapeHTML(qbNotebookStatusLabel(item.status))}</span></header><div class="card-meta-grid"><span>Disciplina: ${escapeHTML(item.disciplina||'-')}</span><span>Assunto: ${escapeHTML(item.assunto||'-')}</span><span>Motivo: ${escapeHTML(qbErrorReasonLabel(item.motivo || item.motivoErro))}</span><span>Quantidade de erros: ${Number(item.quantidadeErros)||1}</span><span>Status: ${escapeHTML(qbNotebookStatusLabel(item.status))}</span><span>Último erro: ${escapeHTML(qbNotebookLastErrorDate(item))}</span></div><div class="actions"><button data-qb-error-review="${escapeHTML(item.id)}" type="button">Revisar agora</button>${item.status !== 'dominado' ? `<button class="secondary-button" data-qb-error-status="dominado" data-qb-error-id="${escapeHTML(item.id)}" type="button">Marcar como dominada</button>` : ""}</div><details class="qb-error-details"><summary>Ver detalhes do registro</summary><div class="card-meta-grid"><span>Banca: ${escapeHTML(item.banca||'-')}</span><span>Cargo: ${escapeHTML(item.cargo||'-')}</span><span>Resposta marcada: ${escapeHTML(item.respostaMarcada||'-')}</span><span>Registrado em: ${escapeHTML(qbNotebookLastErrorDate(item))}</span><span>Última revisão: ${item.ultimaRevisao ? new Date(item.ultimaRevisao).toLocaleString('pt-BR') : "-"}</span></div><p>${escapeHTML(item.enunciado||'Enunciado não cadastrado.')}</p><details><summary>Mostrar gabarito e justificativa</summary><p><strong>Gabarito:</strong> ${escapeHTML(item.gabaritoCorreto||'-')}</p><p><strong>Justificativa/fundamento:</strong> ${escapeHTML(item.justificativa||item.fundamento||'Sem justificativa cadastrada')}</p></details></details></article>`).join("") : "Nenhum registro no caderno de erros."; }
...
function qbStartNotebookTraining(items = qbFilteredNotebook().filter(i=>i.status!=="dominado")) { const questions = items.map(qbNotebookQuestionForItem).filter(Boolean); if (!questions.length) return alert("Nenhuma questão pendente/revisada do caderno corresponde ao banco atual."); qbStart(questions, { mode:"errorNotebook" }); }
