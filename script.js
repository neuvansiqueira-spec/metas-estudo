...
function buildFullPerformanceReportSvg(payload, metadata = {}) {
  ...
  const H = Math.max(1050, 260 + rows.length * rowH + 80);
  let y = 210;
...
async function exportFullReportPng(payload, filenameBase) { const svg = buildFullPerformanceReportSvg(payload, { period: payload.filters?.periodLabel || payload.filters?.mode || 'Período selecionado' }); const height = Math.max(1400, Math.min(6000, Math.round((Number(svg.match(/viewBox="0 0 1600 (\d+)/)?.[1]) || 1050) * 1.5))); const blob = await svgToPngBlob(svg, { width: 2400, height }); downloadGeneratedFile(blob, `${filenameBase}.png`); return blob; }
...
async function handlePerformanceExportAction(action, trigger) {
  ...
  try {
    ...
    if (action === 'png' || action === 'share') {
      ...
      const height = Math.max(1400, Math.min(6000, Math.round((Number(svg.match(/viewBox=\"0 0 1600 (\\d+)/)?.[1]) || 1050) * 1.5)));
      const blob = await svgToPngBlob(svg, { width: 2400, height });
