...
function renderQbDiagnostics() { if (!elements.qbDiagnostics) return; const packages = qbSyllabusPackages(); const withQuestions = packages.filter((pkg)=>pkg.questions.length).map((pkg)=>`${pkg.discipline} (${pkg.questions.length})`); const withoutQuestions = packages.filter((pkg)=>!pkg.questions.length).map((pkg)=>pkg.discipline); const missingSubjects = packages.flatMap((pkg)=>pkg.missing.map((item)=>`${pkg.discipline} — ${item.subject || item.assunto || item.topic || "Assunto"}`)); elements.qbDiagnostics.innerHTML = `<article class="question-bank-item"><header><h4>Diagnóstico do edital no banco</h4><span class="badge neutral">${packages.length} disciplina(s)</span></header><div class="card-meta-grid"><span>Disciplinas do edital com questões: ${escapeHTML(withQuestions.join(", ") || "nenhuma")}</span><span>Disciplinas do edital sem questões: ${escapeHTML(withoutQuestions.join(", ") || "nenhuma")}</span><span>Assuntos do edital sem questões: ${escapeHTML(missingSubjects.join(", ") || "nenhum")}</span></div></article>`; }
