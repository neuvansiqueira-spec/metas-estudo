...
function renderNetByDisciplineChart(rows = [], period = {}) {
  ...
  return `<section class="performance-chart performance-net-chart" aria-label="Líquido por disciplina"><p class="chart-explainer">Foram registradas questões em ${withQuestions.length} ${withQuestions.length === 1 ? 'disciplina' : 'disciplinas'}. ${explanation}</p><p class="chart-legend">Escala: centro = zero; máximo absoluto = ${maxAbs} ponto(s). Unidade: líquido Cebraspe (acertos - erros) no ${escapeHTML(analyticsPeriodLabel(period))}.</p><div class="net-axis" aria-hidden="true"><span>Negativo</span><b>0</b><span>Positivo</span></div><div class="performance-net-list">${withQuestions.map((r)=>{ const net=Number(r.net)||0, sample=Number(r.questions)||0, side=net>=0?'positive':'negative', width=Math.max(4, Math.abs(net)/maxAbs*50); return `<article class="net-discipline-row ${side}"><strong>${escapeHTML(r.discipline)}</strong><div class="net-bar-line"><span class="net-left">${net<0?`<i style="width:${width}%"></i>`:''}</span><b class="net-zero"></b><span class="net-right">${net>=0?`<i style="width:${width}%"></i>`:''}</span></div><p>Líquido: <strong>${net>0?'+':''}${net}</strong></p><small>${Number(r.correct)||0} acertos • ${Number(r.wrong)||0} erros • ${Number(r.blank)||0} brancos</small><small>Amostra: ${sample} ${sample === 1 ? 'questão' : 'questões'}</small></article>`; }).join('')}</div>${renderSupportTable(['Disciplina','Acertos','Erros','Brancos','Líquido','Amostra'], withQuestions.map((r)=>[r.discipline, r.correct, r.wrong, r.blank, r.net, r.questions]))}</section>`;
}
