...
const defaultPlanning = { config: { examDate: "", scaleType: "24x72", scaleNotes: "", shiftHours: 1, restHours: 5, normalHours: 2.5, minWeeklyHours: 10, idealWeeklyHours: 18, weeklyTopics: 8, safetyDays: 7, disciplinesPerDay: 2, disciplinesPerWeek: 6, disciplinesPerMonth: 10, topicsPerDay: 3, topicsPerWeek: 8, topicsPerMonth: 40 }, availability: {}, weeklyGoals: [], forecasts: {} };
...
function generateGoalsForDate(date, opts = {}) {
  ...
  const c = planningConfig();
  const baseMaxGoals = opts.maxGoals || (dayType === "plantão" ? 2 : dayType === "folga" || dayType === "estudo forte" ? 6 : dayType === "estudo leve" ? 3 : 4);
...
function generateDailyGoals() {
  try {
    ...
    showDailyGoalMessage(`${chosen.length} meta(s) gerada(s) para ${date}, com ${disciplines} disciplina(s) diferente(s). Limitação aplicada: ${availability.type} (${availability.hours || 0}h disponíveis), até ${planningConfig().disciplinesPerDay} disciplina(s) e ${planningConfig().topicsPerDay} assunto(s).${generationShortageMessage(chosen, planningConfig().topicsPerDay, "o dia")}`, "success");
