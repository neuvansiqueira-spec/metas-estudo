...
function qbStartNotebookTraining(items = qbFilteredNotebook().filter(i=>i.status!=="dominado")) { const questions = items.map(qbNotebookQuestionForItem).filter(Boolean); if (!questions.length) return alert("Nenhuma questão disponível para revisão. Verifique se o Caderno possui registros e se as questões ainda existem no banco."); qbStart(questions, { mode:"errorNotebook" }); if (typeof showView === "function") showView("treino-questoes"); }
...
function qbStart(items = qbFilteredQuestions(), options = {}) { if (!items.length) return alert(qbSelectedZeroDisciplineMessage() || "Nenhuma questão encontrada com os filtros atuais."); if (elements.qbShuffleTraining?.checked) items = qbShuffle(items); items = items.slice(0, Math.max(1, Number(elements.qbTrainingLimit?.value) || items.length)); const mode=options.mode || "default"; questionBankTraining = { id:createId(), createdAt:new Date().toISOString(), index:0, items, answers:{}, cadernoRegistrado:{}, mode }; if (elements.treinoQuestoesTitle) elements.treinoQuestoesTitle.textContent = mode === "errorNotebook" ? "Revisão do Caderno de Erros" : "Treino do Banco de Questões"; if (typeof showView === "function") showView("treino-questoes"); elements.qbTrainingPanel.hidden = false; elements.qbResultPanel.hidden = true; qbRenderQuestion(); }
