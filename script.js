...
function renderQuestionBank() { if (!elements.qbStats) return; normalizeStoredQuestionBank(); const bank = state.questionBank || [], sessions = state.questionBankSessions || [], last = sessions[0]; qbRenderCascadingFilters(); renderQbSyllabusPackages(); const filteredTotal = qbFilteredQuestions().length; elements.qbStats.innerHTML = [["Questões no banco", bank.length], ["Questões filtradas", filteredTotal], ["Disciplinas", qbUnique(bank.map(q=>q.disciplina)).length], ["Assuntos", qbUnique(bank.map(q=>q.assunto)).length], ["Com gabarito", bank.filter(qbHasKey).length], ["Treinos realizados", sessions.length], ["Último treino", last ? new Date(last.createdAt).toLocaleString("pt-BR") : "Nenhum"]].map(([a,b])=>`<article class="stat-card"><span>${a}</span><strong>${escapeHTML(b)}</strong></article>`).join(""); }
