...
function qbStartNotebookTraining(items = qbFilteredNotebook().filter(i=>i.status!=="dominado")) { const questions = items.map(qbNotebookQuestionForItem).filter(Boolean); if (!questions.length) return alert("Nenhuma questão disponível para revisão. Verifique se há registros na Revisão de Erros."); qbStart(questions, { mode:"errorNotebook" }); if (typeof showView === "function") showView("treino-questoes"); }
...
function qbRenderInactiveTrainingNotice() {
  ...
  if (elements.treinoQuestoesTitle) elements.treinoQuestoesTitle.textContent = "Treino/Revisão Ativa";
...
function renderView(viewId) {
  const renderers = {
    ...
    "treino-questoes": qbRenderTrainingView,
