...
const defaultState = { subjects: [], studies: [], edital: { pdf: null }, syllabusItems: [], schedulableSettings: {}, dailyGoals: [], questionLogs: [], simulados: [], settings: { defaultMockGoal: 92 }, planning: { config: {}, calendar: [], weeklyGoals: [], forecasts: [] }, timeLogs: [] };
...
state.migrations ||= {};
state.planning ||= { config: {}, calendar: [], weeklyGoals: [], forecasts: [] };
state.planning.config ||= {}; state.planning.calendar ||= []; state.planning.weeklyGoals ||= []; state.planning.forecasts ||= [];
state.timeLogs ||= [];
...
const elements = {
  ...
  planningConfigForm: $("#planningConfigForm"), planningExamDate: $("#planningExamDate"), planningScaleType: $("#planningScaleType"), planningScaleNotes: $("#planningScaleNotes"), planningHoursPlantao: $("#planningHoursPlantao"), planningHoursFolga: $("#planningHoursFolga"), planningHoursNormal: $("#planningHoursNormal"), planningMinWeeklyHours: $("#planningMinWeeklyHours"), planningIdealWeeklyHours: $("#planningIdealWeeklyHours"), planningWeeklyTopics: $("#planningWeeklyTopics"), planningSafetyDays: $("#planningSafetyDays"), availabilityCalendar: $("#availabilityCalendar"), planningForecast: $("#planningForecast"), planningForecastAlert: $("#planningForecastAlert"), weeklyPlanning: $("#weeklyPlanning"), weeklyPlanningAlert: $("#weeklyPlanningAlert"), timeHistorySummary: $("#timeHistorySummary"), timeHistoryTable: $("#timeHistoryTable"), planningTotalHours: $("#planningTotalHours"), planningAvgTopicTime: $("#planningAvgTopicTime"), planningForecastDate: $("#planningForecastDate"), planningDaysToExam: $("#planningDaysToExam"), planningSituation: $("#planningSituation")
};
...
function replaceState(nextState) { Object.keys(state).forEach((key) => delete state[key]); Object.assign(state, { ...structuredClone(defaultState), ...(nextState || {}) }); state.edital = { ...defaultState.edital, ...(state.edital || {}) }; state.syllabusItems ||= []; state.schedulableSettings ||= {}; state.dailyGoals ||= []; state.questionLogs ||= []; state.simulados ||= []; state.settings ||= {}; state.settings.defaultMockGoal ||= 92; state.planning ||= { config: {}, calendar: [], weeklyGoals: [], forecasts: [] }; state.timeLogs ||= []; }
...
function mergeBackupData(data = {}) {
  ...
  mergeArrays(state.timeLogs, data.timeLogs || [], (item) => item.id || [item.date, item.discipline, item.subject, item.minutes].join("|"));
  mergeArrays(state.planning.calendar, data.planning?.calendar || [], (item) => item.date);
  mergeArrays(state.planning.weeklyGoals, data.planning?.weeklyGoals || [], (item) => item.weekStart || item.id);
  state.planning.config = { ...(data.planning?.config || {}), ...state.planning.config };
  state.planning.forecasts = data.planning?.forecasts || state.planning.forecasts || [];
...
const availabilityDefaults = { "plantão": 1, "folga": 5, "dia normal": 2.5, "indisponível": 0, "estudo leve": 1.5, "estudo forte": 6 };
function weekStartISO(dateString = todayISO()) { const d = parseDate(dateString); d.setDate(d.getDate() - d.getDay()); return d.toISOString().slice(0,10); }
function daysBetween(a,b){ return Math.ceil((parseDate(b)-parseDate(a))/86400000); }
function getAvailability(date = todayISO()) { return state.planning.calendar.find((d)=>d.date===date) || { date, type: "dia normal", hours: Number(state.planning.config.hoursNormal ?? 2.5) || 2.5 }; }
function ensurePlanningCalendar(days = 35) { const known = new Map(state.planning.calendar.map((d)=>[d.date,d])); for (let i=0;i<days;i++){ const date=addDays(todayISO(), i); if(!known.has(date)) state.planning.calendar.push({ date, type:"dia normal", hours:Number(state.planning.config.hoursNormal ?? 2.5) || 2.5 }); } state.planning.calendar.sort((a,b)=>a.date.localeCompare(b.date)); }
function timeEntries(){
  const fromStudies = state.studies.map((s)=>({ id:s.id, date:s.date, discipline:subjectNameById(s.subjectId), subject:s.topic, minutes:Number(s.minutes)||0, plannedMinutes:Number(s.plannedMinutes)||0, type:s.type||"Estudo", status:s.topicStatus||s.status||"Iniciado", notes:s.difficultyNotes||s.notes||"" }));
  const fromQuestions = state.questionLogs.filter((q)=>Number(q.timeMinutes)>0).map((q)=>({ id:q.id, date:q.date, discipline:q.discipline, subject:q.subject, minutes:Number(q.timeMinutes)||0, plannedMinutes:0, type:q.trainingType||"Questões", status:"Iniciado", notes:q.notes||"" }));
  return [...fromStudies, ...fromQuestions, ...(state.timeLogs||[])];
}
function planningStats(){
  const entries=timeEntries(); const studiedMinutes=entries.reduce((a,e)=>a+(Number(e.minutes)||0),0); const total=state.syllabusItems.length; const done=state.syllabusItems.filter(completedStatus).length; const pending=Math.max(0,total-done); const avgMin=done? studiedMinutes/done : (entries.length? studiedMinutes/Math.max(1,new Set(entries.map(e=>canonical(e.discipline+"|"+e.subject))).size):60); const remainingMinutes=pending*avgMin; const weekSet=new Set(entries.map(e=>weekStartISO(e.date))); const avgTopicsWeek=weekSet.size? done/weekSet.size:0; const avgHoursDay=Math.max(0.25, entries.length? studiedMinutes/60/Math.max(1, new Set(entries.map(e=>e.date)).size): (Number(state.planning.config.idealWeeklyHours)||10)/7); const completion=addDays(todayISO(), Math.ceil(remainingMinutes/60/avgHoursDay)); const exam=state.planning.config.examDate || state.edital.examDate || ""; const diff=exam?daysBetween(completion, exam):null; return { entries, studiedMinutes,total,done,pending,percent:total?Math.round(done/total*100):0,avgTopicsWeek,avgMin,estimatedTotal:total*avgMin,remainingMinutes,completion,exam,diff };
}
function renderPlanning(){ if(!elements.planningConfigForm) return; ensurePlanningCalendar(); const c=state.planning.config; elements.planningExamDate.value=c.examDate||state.edital.examDate||""; elements.planningScaleType.value=c.scaleType||"24x72"; elements.planningScaleNotes.value=c.scaleNotes||""; elements.planningHoursPlantao.value=c.hoursPlantao??1; elements.planningHoursFolga.value=c.hoursFolga??5; elements.planningHoursNormal.value=c.hoursNormal??2.5; elements.planningMinWeeklyHours.value=c.minWeeklyHours??8; elements.planningIdealWeeklyHours.value=c.idealWeeklyHours??14; elements.planningWeeklyTopics.value=c.weeklyTopics??5; elements.planningSafetyDays.value=c.safetyDays??7;
  const types=Object.keys(availabilityDefaults); elements.availabilityCalendar.innerHTML=`<table><thead><tr><th>Data</th><th>Tipo</th><th>Horas</th></tr></thead><tbody>${state.planning.calendar.slice(0,35).map(d=>`<tr><td>${d.date}</td><td><select data-availability-type="${d.date}">${types.map(t=>`<option ${d.type===t?'selected':''}>${t}</option>`).join('')}</select></td><td><input data-availability-hours="${d.date}" type="number" min="0" step="0.5" value="${d.hours}"></td></tr>`).join('')}</tbody></table>`;
  const st=planningStats(); const weekStart=weekStartISO(); const weekHours=state.planning.calendar.filter(d=>d.date>=weekStart&&d.date<addDays(weekStart,7)).reduce((a,d)=>a+Number(d.hours||0),0); const safetyExam=st.exam?addDays(st.exam, -Number(c.safetyDays||0)):""; const daysToSafe=safetyExam?Math.max(1, daysBetween(todayISO(), safetyExam)):30; const suggestedTopics=Math.min(st.pending, Math.max(1, Math.ceil((st.remainingMinutes/60)/(daysToSafe/7)/Math.max(st.avgMin/60,0.5)))); const suggestedHoursDay=weekHours? (weekHours/7).toFixed(1):"0"; const delayed=st.exam && st.diff<0;
  elements.planningForecast.innerHTML=[["Total de assuntos",st.total],["Assuntos concluídos",st.done],["Assuntos pendentes",st.pending],["Edital estudado",st.percent+"%"],["Média assuntos/semana",st.avgTopicsWeek.toFixed(1)],["Média tempo/assunto",formatHours(st.avgMin)],["Horas estimadas totais",formatHours(st.estimatedTotal)],["Horas já estudadas",formatHours(st.studiedMinutes)],["Horas restantes",formatHours(st.remainingMinutes)],["Previsão",st.completion],["Diferença para prova",st.diff===null?"-":st.diff+" dia(s)"]].map(([l,v])=>`<article class="stat-card"><span>${l}</span><strong>${v}</strong></article>`).join(''); elements.planningForecastAlert.textContent=st.exam?(delayed?"No ritmo atual, o edital não será concluído antes da prova.":"No ritmo atual, o edital será concluído antes da prova."):"Informe a data da prova para comparar a previsão.";
  elements.weeklyPlanning.innerHTML=[["Horas disponíveis na semana",weekHours.toFixed(1)+"h"],["Assuntos sugeridos",suggestedTopics],["Horas sugeridas por dia",suggestedHoursDay+"h"],["Meta mínima",(c.minWeeklyHours??8)+"h"],["Meta ideal",(c.idealWeeklyHours??14)+"h"]].map(([l,v])=>`<article class="stat-card"><span>${l}</span><strong>${v}</strong></article>`).join(''); elements.weeklyPlanningAlert.textContent=delayed?"Alerta: planejamento atrasado. Priorize folgas e estudo forte.":"Planejamento em dia ou adiantado. Ajuste manualmente se necessário."; state.planning.forecasts=[{...st, updatedAt:new Date().toISOString()}]; state.planning.weeklyGoals=[{weekStart, availableHours:weekHours, suggestedTopics, suggestedHoursPerDay:Number(suggestedHoursDay), minHours:Number(c.minWeeklyHours)||0, idealHours:Number(c.idealWeeklyHours)||0, status:delayed?"atrasado":"em dia"}];
  const entries=st.entries; const byDisc=entries.reduce((a,e)=>(a[e.discipline]=(a[e.discipline]||0)+e.minutes,a),{}); elements.timeHistorySummary.innerHTML=[["Hoje",formatHours(entries.filter(e=>e.date===todayISO()).reduce((a,e)=>a+e.minutes,0))],["Semana",formatHours(entries.filter(e=>isSameWeek(e.date)).reduce((a,e)=>a+e.minutes,0))],["Edital",formatHours(st.studiedMinutes)],["Disciplinas",Object.keys(byDisc).length]].map(([l,v])=>`<article class="stat-card"><span>${l}</span><strong>${v}</strong></article>`).join(''); elements.timeHistoryTable.innerHTML=`<table><thead><tr><th>Data</th><th>Disciplina</th><th>Assunto</th><th>Tempo</th><th>Tipo</th><th>Status</th><th>Observações</th></tr></thead><tbody>${entries.sort((a,b)=>b.date.localeCompare(a.date)).slice(0,50).map(e=>`<tr><td>${e.date}</td><td>${escapeHTML(e.discipline)}</td><td>${escapeHTML(e.subject)}</td><td>${formatHours(e.minutes)}</td><td>${escapeHTML(e.type)}</td><td>${escapeHTML(e.status)}</td><td>${escapeHTML(e.notes||'-')}</td></tr>`).join('')}</tbody></table>`; }
...
function renderDashboard() {
  ...
  const ps = planningStats(); if (elements.planningTotalHours) { elements.planningTotalHours.textContent = formatHours(ps.studiedMinutes); elements.planningAvgTopicTime.textContent = formatHours(ps.avgMin); elements.planningForecastDate.textContent = ps.completion; elements.planningDaysToExam.textContent = ps.exam ? Math.max(0, daysBetween(todayISO(), ps.exam)) : "-"; elements.planningSituation.textContent = ps.exam ? (ps.diff < 0 ? "atrasado" : ps.diff <= Number(state.planning.config.safetyDays || 7) ? "em dia" : "adiantado") : "-"; }
...
function renderHistory() { elements.historyBody.innerHTML = ""; [...state.studies].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 20).forEach((study) => { const row = document.createElement("tr"); row.innerHTML = `<td>${study.date}</td><td>${escapeHTML(subjectNameById(study.subjectId))}</td><td>${escapeHTML(study.topic)}</td><td>${study.minutes}</td><td>${study.questions}</td><td>${study.correct}</td><td>${study.wrong}</td><td>${study.blank}</td><td>${study.plannedMinutes || 0}</td><td>${escapeHTML(study.topicStatus || "Iniciado")}</td><td>${escapeHTML(study.difficultyNotes || "-")}</td>`; elements.historyBody.appendChild(row); }); }
...
elements.studyForm.addEventListener("submit", (event) => { event.preventDefault(); if (!elements.studySubject.value) return alert("Cadastre uma disciplina antes de registrar o estudo."); const questions = Number(elements.questionsDone.value); const correct = Number(elements.correctAnswers.value); const wrong = Number(elements.wrongAnswers.value); const blank = Number(elements.blankAnswers.value); if (correct + wrong + blank !== questions) return alert("A soma de acertos, erros e brancos deve ser igual ao total de questões feitas."); state.studies.push({ id: createId(), date: elements.studyDate.value, subjectId: elements.studySubject.value, topic: elements.studyTopic.value.trim(), minutes: Number(elements.studyMinutes.value), plannedMinutes: Number(elements.studyPlannedMinutes?.value)||0, realMinutes: Number(elements.studyMinutes.value), topicStatus: elements.studyTopicStatus?.value || "Iniciado", status: elements.studyTopicStatus?.value || "Iniciado", difficultyNotes: elements.studyDifficultyNotes?.value.trim() || "", questions, correct, wrong, blank }); elements.studyForm.reset(); elements.studyDate.value = todayISO();
elements.goalDate.value = todayISO();
...
function makeGoal(item, date, type) {
  const day = getAvailability(date); const factor = day.type === "plantão" ? 0.6 : day.type === "folga" || day.type === "estudo forte" ? 1.3 : day.type === "indisponível" ? 0.2 : 1; const minutes = Math.max(15, Math.round((type === "Questões" ? 30 : item.priority === "Alta" ? 60 : 45) * factor));
...
function generateDailyGoals() {
  try {
    ...
    const add = (type, predicate) => { const item = pickCandidate(candidates, used, predicate); if (item && chosen.length < (getAvailability(date).type === "plantão" ? 2 : getAvailability(date).type === "folga" || getAvailability(date).type === "estudo forte" ? 6 : 5)) { used.add(item.id); chosen.push(makeGoal(item, date, type)); } };
    ...
    while (chosen.length < Math.min(getAvailability(date).type === "plantão" ? 2 : getAvailability(date).type === "folga" || getAvailability(date).type === "estudo forte" ? 6 : 4, candidates.length)) {
...
elements.goalForm.addEventListener("submit", (event) => { event.preventDefault(); const item = getSyllabusById(elements.goalSyllabusItem.value); if (!item) return alert("Selecione um assunto do edital verticalizado."); state.dailyGoals.push({ id: createId(), date: elements.goalDate.value, discipline: elements.goalDiscipline.value, syllabusItemId: item.id, subject: item.subject, type: elements.goalType.value, minutes: Number(elements.goalMinutes.value), plannedMinutes: Number(elements.goalMinutes.value), actualMinutes: Number(elements.goalActualMinutes?.value)||0, studyStatus: elements.goalStudyStatus?.value || "Iniciado", priority: elements.goalPriority.value, status: elements.goalStatus.value, notes: elements.goalNotes.value.trim() }); elements.goalForm.reset(); elements.goalDate.value = todayISO(); render(); });
...
elements.planningConfigForm?.addEventListener("submit", (event)=>{ event.preventDefault(); state.planning.config = { examDate: elements.planningExamDate.value, scaleType: elements.planningScaleType.value, scaleNotes: elements.planningScaleNotes.value.trim(), hoursPlantao:Number(elements.planningHoursPlantao.value)||0, hoursFolga:Number(elements.planningHoursFolga.value)||0, hoursNormal:Number(elements.planningHoursNormal.value)||0, minWeeklyHours:Number(elements.planningMinWeeklyHours.value)||0, idealWeeklyHours:Number(elements.planningIdealWeeklyHours.value)||0, weeklyTopics:Number(elements.planningWeeklyTopics.value)||0, safetyDays:Number(elements.planningSafetyDays.value)||0 }; if (state.planning.config.examDate) state.edital.examDate = state.planning.config.examDate; render(); });
elements.availabilityCalendar?.addEventListener("change", (event)=>{ const date=event.target.dataset.availabilityType || event.target.dataset.availabilityHours; if(!date) return; const entry = state.planning.calendar.find((d)=>d.date===date); if(!entry) return; if(event.target.dataset.availabilityType){ entry.type=event.target.value; entry.hours=availabilityDefaults[entry.type] ?? entry.hours; } else entry.hours=Number(event.target.value)||0; render(); });
