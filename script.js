...
function analyticsSection(id, title, summary, html, open = false) { return `<details class="analytics-section" data-analytics-section="${escapeHTML(id)}"${open ? ' open' : ''}><summary><span>${escapeHTML(title)}</span><small>${escapeHTML(summary || '')}</small></summary><div class="analytics-section-content">${html}</div></details>`; }
