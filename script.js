...
const defaultState = { subjects: [], studies: [], edital: { pdf: null }, syllabusItems: [], schedulableSettings: {}, dailyGoals: [], questionLogs: [], simulados: [], planning: structuredClone(defaultPlanning), settings: { defaultMockGoal: 92 } };
...
function replaceState(nextState) { Object.keys(state).forEach((key) => delete state[key]); Object.assign(state, { ...structuredClone(defaultState), ...(nextState || {}) }); state.edital = { ...defaultState.edital, ...(state.edital || {}) }; state.syllabusItems ||= []; state.schedulableSettings ||= {}; state.dailyGoals ||= []; state.questionLogs ||= []; state.simulados ||= []; state.planning = { ...structuredClone(defaultPlanning), ...(state.planning || {}), config: { ...defaultPlanning.config, ...(state.planning?.config || {}) }, availability: { ...(state.planning?.availability || {}) }, weeklyGoals: state.planning?.weeklyGoals || [], forecasts: { ...(state.planning?.forecasts || {}) } }; state.settings ||= {}; state.settings.defaultMockGoal ||= 92; }
...
function getStudyTimeLogs() {
  const studyLogs = state.studies.map((study) => ({ date: study.date, discipline: subjectNameById(study.subjectId), subject: study.topic, minutes: Number(study.minutes) || 0, plannedMinutes: Number(study.plannedMinutes) || 0, type: "Estudo", status: study.topicStatus || "Iniciado", notes: study.difficultyNotes || "" }));
...
function renderPlanning() {
  ...
  elements.timeHistoryBody.innerHTML = logs.slice(0,50).map((l)=>`<tr><td>${l.date}</td><td>${escapeHTML(l.discipline)}</td><td>${escapeHTML(l.subject)}</td><td>${l.minutes} min</td><td>${escapeHTML(l.type)}</td><td>${escapeHTML(l.status)}</td><td>${escapeHTML(l.notes||"-")}</td></tr>`).join("");
}
...
elements.studyForm.addEventListener("submit", (event) => { event.preventDefault(); if (!elements.studySubject.value) return alert("Cadastre uma disciplina antes de registrar o estudo."); const questions = Number(elements.questionsDone.value); const correct = Number(elements.correctAnswers.value); const wrong = Number(elements.wrongAnswers.value); const blank = Number(elements.blankAnswers.value); if (correct + wrong + blank !== questions) return alert("A soma de acertos, erros e brancos deve ser igual ao total de questões feitas."); state.studies.push({ id: createId(), date: elements.studyDate.value, subjectId: elements.studySubject.value, topic: elements.studyTopic.value.trim(), minutes: Number(elements.studyMinutes.value), plannedMinutes: Number(elements.studyPlannedMinutes?.value) || 0, topicStatus: elements.studyTopicStatus?.value || "Iniciado", difficultyNotes: elements.studyDifficultyNotes?.value.trim() || "", questions, correct, wrong, blank }); elements.studyForm.reset(); elements.studyDate.value = todayISO();
elements.goalDate.value = todayISO();
