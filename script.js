...
function renderDashboard() {
  ...
  if (elements.dashboardMinWeeklyHours) elements.dashboardMinWeeklyHours.textContent = `${planningConfig().minWeeklyHours || 0}h`; if (elements.dashboardIdealWeeklyHours) elements.dashboardIdealWeeklyHours.textContent = `${planningConfig().idealWeeklyHours || 0}h`; if (elements.dashboardProblemDiscipline) elements.dashboardProblemDiscipline.textContent = problemQuestionDiscipline();
...
function generateDailyGoals() {
  try {
    ...
    showDailyGoalMessage(`${chosen.length} meta(s) gerada(s) para ${date}, com ${disciplines} disciplina(s) diferente(s). Limitação aplicada: ${availability.type} (${availability.hours || 0}h disponíveis).`, "success");
