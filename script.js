...
const openMaterialDetailIds = new Set();
function materialCardHTML(m) {
  ...
  const isOpen = openMaterialDetailIds.has(m.id);
  const normalized = normalizeMaterialEstimateFields(m);
  const carga = normalized.estimatedMinutes ? `<p class="item-meta">Carga estimada: ${formatHours(normalized.estimatedMinutes)}</p>` : "";
  const tags = materialTagsArray(m.tags).join(", ") || "Sem tags";
  return `<article class="syllabus-card material-card" data-material-card="${m.id}"><header class="material-card-summary"><div><h3>${escapeHTML(m.title)}</h3><p class="item-meta">${escapeHTML(m.discipline || "Sem disciplina")}</p><p class="item-meta">${escapeHTML(m.subject || "Sem assunto")}</p><p class="item-meta">${escapeHTML(m.type || "Material")} • ${escapeHTML(origem)}</p>${carga}</div><button type="button" class="secondary-button material-details-toggle" data-toggle-material-details="${m.id}" aria-expanded="${isOpen ? "true" : "false"}" aria-controls="material-details-${m.id}">${isOpen ? "Fechar detalhes" : "Abrir detalhes"}</button></header><div id="material-details-${m.id}" class="material-card-details" data-material-details="${m.id}" ${isOpen ? "" : "hidden"}><div class="card-meta-grid"><span>Título: ${escapeHTML(m.title)}</span><span>Disciplina: ${escapeHTML(m.discipline)}</span><span>Assunto: ${escapeHTML(m.subject)}</span><span>Módulo: ${escapeHTML(modulo)}</span><span>Formato: ${escapeHTML(m.factoryFormat || m.type || "-")}</span><span>Tipo: ${escapeHTML(m.type || "-")}</span><span>Origem: ${escapeHTML(origem)}</span><span>Origem cadastrada: ${escapeHTML(m.origin || "-")}</span><span>Data: ${formatDateBR(m.date)}</span><span>Link do material: ${m.link ? `<a href="${escapeHTML(m.link)}" target="_blank" rel="noopener">${escapeHTML(m.link)}</a>` : "-"}</span><span>Tags: ${escapeHTML(tags)}</span><span>Observações: ${escapeHTML(m.notes || "Sem observações")}</span></div>${materialEstimateSummaryHTML(m)}${materialEstimateFormHTML(m)}<div class="card-actions"><button type="button" data-open-material="${m.id}">Abrir</button><button type="button" data-use-material-study="${m.id}">Usar no estudo</button><button type="button" data-edit-material="${m.id}">Editar</button><button class="danger" type="button" data-delete-material="${m.id}">Excluir</button></div></div></article>`;
}
function toggleMaterialDetails(button) {
  const id = button?.dataset?.toggleMaterialDetails;
  if (!id) return;
  const card = button.closest(`[data-material-card="${CSS.escape(id)}"]`);
  const details = card?.querySelector(`[data-material-details="${CSS.escape(id)}"]`);
  if (!details) return;
  const willOpen = details.hidden;
  details.hidden = !willOpen;
  button.textContent = willOpen ? "Fechar detalhes" : "Abrir detalhes";
  button.setAttribute("aria-expanded", willOpen ? "true" : "false");
  if (willOpen) openMaterialDetailIds.add(id); else openMaterialDetailIds.delete(id);
}
