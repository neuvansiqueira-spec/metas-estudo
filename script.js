...
function qbRenderErrorNotebook() { if (!elements.qbErrorStats) return; qbRenderNotebookFilters(); const stats=qbNotebookStats(), all=qbFilteredNotebook(), list=all.slice(0, QB_RENDER_LIMIT), hidden=Math.max(0, all.length - list.length); elements.qbErrorStats.innerHTML = [["Total de erros",stats.total],["Disciplinas com mais erros",stats.disciplinas],["Assuntos com mais erros",stats.assuntos],["Questões dominadas",stats.dominadas],["Questões pendentes",stats.pendentes]].map(([a,b])=>`<article class="stat-card"><span>${a}</span><strong>${escapeHTML(b)}</strong></article>`).join(""); elements.qbErrorNotebookList.innerHTML = all.length ? `<p class="item-meta">Exibindo ${list.length} de ${all.length} registro(s).${hidden ? " Ajuste os filtros para refinar." : ""}</p>` + list.map((item)=>`<article class="question-bank-item"><header><h4>${escapeHTML(item.disciplina)} — ${escapeHTML(item.assunto)}</h4><span class="badge ${item.status==='dominado'?'success':item.status==='revisado'?'neutral':'warn'}">${escapeHTML(item.status)}</span></header><div class="card-meta-grid"><span>Banca: ${escapeHTML(item.banca||'-')}</span><span>Cargo: ${escapeHTML(item.cargo||'-')}</span><span>Motivo: ${escapeHTML(qbErrorReasonLabel(item.motivo || item.motivoErro))}</span><span>Erros nesta questão: ${Number(item.quantidadeErros)||1}</span><span>Resposta: ${escapeHTML(item.respostaMarcada||'-')}</span><span>Gabarito: ${escapeHTML(item.gabaritoCorreto||'-')}</span><span>Registrado em: ${new Date(item.dataRegistro).toLocaleString('pt-BR')}</span><span>Última revisão: ${item.ultimaRevisao ? new Date(item.ultimaRevisao).toLocaleString('pt-BR') : "-"}</span></div><p>${escapeHTML(item.enunciado)}</p><p><strong>Justificativa:</strong> ${escapeHTML(item.justificativa||'Sem justificativa cadastrada')}</p><div class="actions"><button class="secondary-button" data-qb-error-status="revisado" data-qb-error-id="${escapeHTML(item.id)}" type="button">Marcar revisada</button><button data-qb-error-status="dominado" data-qb-error-id="${escapeHTML(item.id)}" type="button">Marcar dominada</button></div></article>`).join("") : "Nenhum registro no caderno de erros."; }
...
elements.qbQuestionCard?.addEventListener("click", (event) => { if(!questionBankTraining) return; const answer=event.target.closest("[data-qb-answer]")?.dataset.qbAnswer; if(answer){ const q=questionBankTraining.items[questionBankTraining.index]; questionBankTraining.answers[q.id]=answer; const motivo=qbErrorReason({ ...q, marcado: answer }); if (motivo && questionBankTraining.cadernoRegistrado[q.id] !== `${answer}:${motivo}`) { registrarNoCadernoErros(q, answer, motivo); questionBankTraining.cadernoRegistrado[q.id] = `${answer}:${motivo}`; } if(questionBankTraining.index < questionBankTraining.items.length-1) questionBankTraining.index++; return qbRenderQuestion(); } const nav=event.target.closest("[data-qb-nav]")?.dataset.qbNav; if(nav==="prev"&&questionBankTraining.index>0) questionBankTraining.index--; if(nav==="next"&&questionBankTraining.index<questionBankTraining.items.length-1) questionBankTraining.index++; if(nav) qbRenderQuestion(); if(event.target.closest("[data-qb-finish]")) qbFinish(); });
function qbRefreshAfterFilterChange() { qbRenderCascadingFilters(); if (elements.qbStats) { const filteredTotal = qbFilteredQuestions().length; elements.qbStats.querySelectorAll(".stat-card strong")[1] && (elements.qbStats.querySelectorAll(".stat-card strong")[1].textContent = filteredTotal); } qbPreviewVisible = false; if (elements.qbPreviewSection) elements.qbPreviewSection.hidden = true; if (elements.qbFilteredPreview) elements.qbFilteredPreview.innerHTML = "Clique em “Pré-visualizar” para listar as questões encontradas."; }
