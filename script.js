...
const APP_VERSION = "20260716-calendario-didatico-v9";
...
const elements = {
  ...
  calendarDate: $("#calendarDate"), calendarViewMode: $("#calendarViewMode"), generateWeekGoals: $("#generateWeekGoals"), generateMonthGoals: $("#generateMonthGoals"), disciplineWeightsList: $("#disciplineWeightsList"), goalCalendarStats: $("#goalCalendarStats"), goalCalendarContent: $("#goalCalendarContent"), goalCalendarWeeklyStats: $("#goalCalendarWeeklyStats"), goalCalendarWeeklyContent: $("#goalCalendarWeeklyContent"), goalCalendarMonthlyStats: $("#goalCalendarMonthlyStats"), goalCalendarMonthlyContent: $("#goalCalendarMonthlyContent"), goalCalendarDailyResume: $("#goalCalendarDailyResume"), goalCalendarWeeklyResume: $("#goalCalendarWeeklyResume"), goalCalendarMonthlyResume: $("#goalCalendarMonthlyResume"), exportGoalCalendarPdf: $("#exportGoalCalendarPdf"), exportGoalCalendarCsv: $("#exportGoalCalendarCsv"), exportGoalCalendarImage: $("#exportGoalCalendarImage"), goalCalendarExportStatus: $("#goalCalendarExportStatus"), monthlyTopicGoal: $("#monthlyTopicGoal"), monthlyHourGoal: $("#monthlyHourGoal"), monthlyPlanSummary: $("#monthlyPlanSummary"), todayGoalsTotal: $("#todayGoalsTotal"), weekGoalsTotal: $("#weekGoalsTotal"), weekGoalRate: $("#weekGoalRate"), monthGoalRate: $("#monthGoalRate"), nextGoalLabel: $("#nextGoalLabel"), weekTopDiscipline: $("#weekTopDiscipline"), mostDelayedDiscipline: $("#mostDelayedDiscipline"),
...
function renderGoalCalendar() {
  ...
  const chosen = elements.calendarViewMode?.value || "daily"; document.querySelectorAll("[data-goal-calendar-section]").forEach((section) => { if (section.dataset.goalCalendarSection === chosen) section.open = true; });
...
function buildGoalCalendarCsv(payload) {
  const rows = [["CALENDÁRIO DIDÁTICO DE METAS"], ["Data de referência", formatDateBR(payload.referenceDate)], ["Gerado em", new Date(payload.generatedAt).toLocaleString("pt-BR")], []];
  [["DIA", payload.daily], ["SEMANA", payload.weekly], ["MÊS", payload.monthly]].forEach(([label, period]) => {
...
function buildGoalCalendarPrintHTML(payload) {
  ...
  return `<article id="goalCalendarPrintableReport" class="calendar-print-report"><header><p>PLANEJAMENTO DE ESTUDOS</p><h1>Calendário didático de metas</h1><div>Data de referência: <strong>${formatDateBR(payload.referenceDate)}</strong></div></header>${period("Calendário do dia", payload.daily)}${period("Calendário semanal", payload.weekly)}${period("Calendário mensal", payload.monthly)}<footer>Gerado em ${escapeHTML(new Date(payload.generatedAt).toLocaleString("pt-BR"))}</footer></article>`;
}
...
function exportGoalCalendarPdf() { const payload = buildGoalCalendarExportPayload(); cleanupGoalCalendarPrint(); document.body.insertAdjacentHTML("beforeend", buildGoalCalendarPrintHTML(payload)); document.body.classList.add("calendar-print-mode"); window.addEventListener("afterprint", cleanupGoalCalendarPrint, { once: true }); setGoalCalendarExportStatus("Relatório aberto para salvar como PDF."); setTimeout(() => window.print(), 80); }
function exportGoalCalendarCsv() { const payload = buildGoalCalendarExportPayload(), blob = new Blob([buildGoalCalendarCsv(payload)], { type: "text/csv;charset=utf-8" }); downloadGeneratedFile(blob, `calendario-metas-${payload.referenceDate}.csv`); setGoalCalendarExportStatus("CSV didático gerado com dia, semana e mês."); }
async function exportGoalCalendarImage() { try { const payload = buildGoalCalendarExportPayload(), svg = buildGoalCalendarSvg(payload), viewBoxHeight = Number(svg.match(/viewBox="0 0 1600 (\d+)"/)?.[1]) || 1800, blob = await svgToPngBlob(svg, { width: 2400, height: Math.round(viewBoxHeight * 1.5) }); downloadGeneratedFile(blob, `calendario-metas-${payload.referenceDate}.png`); setGoalCalendarExportStatus("Imagem colorida do calendário gerada."); } catch (error) { setGoalCalendarExportStatus(`Não foi possível gerar a imagem: ${error.message}`, true); } }
