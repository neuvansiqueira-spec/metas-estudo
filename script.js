...
function render() { migrateIncorrectWeakDomains(); renderSubjects(); renderGoalSelectors(); renderQuestionSelectors(); renderPlanning(); renderProgressPanel(); renderDashboard(); renderEdital(); renderSyllabus(); renderSchedulable(); renderDailyGoals(); renderQuestionHistory(); updateQuestionCalculated(); renderMaterials(); updateStudyMaterialOptions(); renderReviews(); renderAlerts(); renderHistory(); renderImportPreview(); renderBackupSummary(); renderSimulados(); saveData(); }
...
function generateDailyGoals() {
  try {
    ...
    const dayType = availabilityForDate(date).type; const maxGoals = dayType === "plantão" ? 2 : dayType === "folga" || dayType === "estudo forte" ? 6 : dayType === "indisponível" ? 1 : 4;
    const used = new Set(); const chosen = [];
    const add = (type, predicate) => { const item = pickCandidate(candidates, used, predicate); if (item && chosen.length < maxGoals) { used.add(item.id); chosen.push(makeGoal(item, date, type)); } };
    ...
    while (chosen.length < Math.min(maxGoals, candidates.length)) {
