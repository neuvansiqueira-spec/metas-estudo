...
function renderQuestionBank(options = {}) { if (!elements.qbStats) return; const heavy = options.heavy !== false; normalizeStoredQuestionBank(); qbRenderCascadingFilters(); if (heavy) { renderQbSyllabusPackages(); renderQbDiagnostics(); renderQbSyllabusVerticalized(); renderQbAccordionSummaries(); qbRenderErrorNotebook(); } qbRenderQuestionBankStats(); const bank = state.questionBank || []; const manage = document.getElementById("qbManageBank"); if (manage && bank.length && !manage.dataset.touched) manage.open = false; }
...
elements.qbErrorNotebookList?.addEventListener("click", (event) => { const btn=event.target.closest("button[data-qb-error-status][data-qb-error-id]"); if (btn) qbSetNotebookStatus(btn.dataset.qbErrorId, btn.dataset.qbErrorStatus); });
