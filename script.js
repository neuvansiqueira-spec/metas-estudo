...
const elements = {
  ...
  dashboardQuestionBankTotal: $("#dashboardQuestionBankTotal"), dashboardQuestionBankSessions: $("#dashboardQuestionBankSessions"), dashboardQuestionBankLast: $("#dashboardQuestionBankLast"),
...
function qbFillSelect(select, values, label) { if (!select) return; const current = select.value; select.innerHTML = `<option value="">${label}</option>` + values.map((v)=>`<option value="${escapeHTML(v)}">${escapeHTML(v)}</option>`).join(""); if (values.includes(current)) select.value = current; }
...
function qbFilteredQuestions() { const search = canonical(elements.qbFilterSearch?.value || ""); return (state.questionBank || []).filter((q)=> (!elements.qbFilterDiscipline?.value || q.disciplina === elements.qbFilterDiscipline.value) && (!elements.qbFilterSubject?.value || q.assunto === elements.qbFilterSubject.value) && (!elements.qbFilterTheme?.value || q.tema === elements.qbFilterTheme.value) && (!elements.qbFilterBoard?.value || q.banca === elements.qbFilterBoard.value) && (!elements.qbFilterYear?.value || String(q.ano) === elements.qbFilterYear.value) && (!search || canonical([q.enunciado,q.disciplina,q.assunto,q.tema,q.banca,q.ano,q.referencia,q.orgao,q.cargo].join(" ")).includes(search))); }
function renderQuestionBank() { if (!elements.qbStats) return; const bank = state.questionBank || [], sessions = state.questionBankSessions || [], last = sessions[0]; qbFillSelect(elements.qbFilterDiscipline, qbUnique(bank.map(q=>q.disciplina)), "Todas"); qbFillSelect(elements.qbFilterSubject, qbUnique(bank.map(q=>q.assunto)), "Todos"); qbFillSelect(elements.qbFilterTheme, qbUnique(bank.map(q=>q.tema)), "Todos"); qbFillSelect(elements.qbFilterBoard, qbUnique(bank.map(q=>q.banca)), "Todas"); qbFillSelect(elements.qbFilterYear, qbUnique(bank.map(q=>q.ano)), "Todos"); elements.qbStats.innerHTML = [["Questões no banco", bank.length], ["Disciplinas", qbUnique(bank.map(q=>q.disciplina)).length], ["Assuntos", qbUnique(bank.map(q=>q.assunto)).length], ["Com gabarito", bank.filter(qbHasKey).length], ["Treinos realizados", sessions.length], ["Último treino", last ? new Date(last.createdAt).toLocaleString("pt-BR") : "Nenhum"]].map(([a,b])=>`<article class="stat-card"><span>${a}</span><strong>${escapeHTML(b)}</strong></article>`).join(""); }
...
[elements.qbFilterDiscipline,elements.qbFilterSubject,elements.qbFilterTheme,elements.qbFilterBoard,elements.qbFilterYear,elements.qbFilterSearch].filter(Boolean).forEach((el)=>el.addEventListener("input", qbPreview));
