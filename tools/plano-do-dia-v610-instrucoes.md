...
A partir de 08/09/2026: uma peça diária, até duas cotas disponíveis do Simulado 3, mais a quantidade de metas automáticas escolhida em Planejamento / Assuntos por dia. Quando as 17 cotas terminarem, aguardar novos assuntos de simulados, sem criar repetições.

## O que o PR faz

O módulo impede novos excessos de metas automáticas. Peça e metas manuais não descontam do limite. Configuração atual tem precedência sobre carimbos antigos. Não há limpeza automática de metas, mudanças em datas anteriores a 08/09 ou execução destes comandos ao carregar o site.

## Comandos de dados

Os assuntos reais não constam do anexo nem deste repositório: os comandos usam as próprias metas identificadas por simulado3V605 e dataOriginalV605 no seu estado.

Execute apenas no console do site, com os dados carregados, cronômetro parado e outras abas do Aldus fechadas durante a operação. Aguarde salvamentos pendentes. Copie o bloco inteiro de Aplicar como um único comando. Ele imprime a tabela antes/depois e verifica a gravação no IndexedDB. Após a mensagem de sucesso, recarregue o site. Nenhum dos comandos foi executado em dados reais nesta entrega.

O backupMetasV610 é preservado no estado e nunca sobrescrito. A remoção alcança somente aulas Delta sem tempo lançado, de 08/09 em diante; concluídas também são preservadas. As cotas disponíveis são pareadas em blocos longos e curtos. Metas com tempo, concluídas, peça e datas anteriores permanecem intactas. Se houver cotas protegidas ocupando vagas, a distribuição se adapta; confira a tabela. O número encontrado é relatado, sem exigir 36/17 nem inventar metas ausentes.

Desfazer restaura aulas retiradas e as datas originais das cotas. Novas metas posteriores são preservadas. Se uma cota redistribuída tiver sido editada ou receber tempo depois, o comando interrompe antes de gravar para não sobrescrever seu trabalho. Ele não reaplica nem sobrescreve backups já usados.
