(() => {
  ...
  const RELEASE_VERSION = "20260727-atualizacao-rapida-segura-v165";
  ...
  function loadCurrentPatches(attempt = 0) {
    ...
    if (!coreReady) {
      if (attempt >= 2400) {
        console.error("[Aldus v165] O núcleo não ficou disponível para carregar as correções complementares.");
        return;
      }
    ...
    Promise.allSettled([
      ...
      loadScript(`update-flow-v165.js?v=${RELEASE_VERSION}`, "update-flow-v165")
    ])).then((results) => {
      results.forEach((result) => {
        if (result.status === "rejected") console.warn("[Aldus v165] Correção complementar não carregada.", result.reason);
      });
    });
  }
