...
test('arquivos públicos permanecem sincronizados e cache usa a nova versão', () => {
  ...
  assert.ok(html.includes(`app-v113.css?v=${packageJson.version}`));
  assert.ok(html.includes(`app-v113.js?v=${packageJson.version}`));
  assert.ok(docsHtml.includes(`app-v113.css?v=${packageJson.version}`));
  assert.ok(docsHtml.includes(`app-v113.js?v=${packageJson.version}`));
