/* V623.4 — calendário exporta dados reais da Fábrica com boa legibilidade. */
...
test('V623.4 treinos elaborados exigem questões importadas', () => {
...
test('V623.4 PDF tem timbre, contraste explícito, Fábrica e meta reconhecida por produto', () => {
  const html=api.buildPrintHtml(report());
  assert.match(html,/class="acv-page-head"/);
...
test('V623.4 imagem usa duas colunas e preserva largura 1600', () => {
...
test('V623.4 Excel tem três abas e informações ampliadas da Fábrica', () => {
...
test('V623.4 continua carregado pela cadeia ativa e espelhado em docs', () => {
  const loader=fs.readFileSync('performance-emergency-v350.js','utf8');
  assert.ok(loader.includes('script.src = "goal-calendar-export-v623.js?v=20260915-calendario-exportacoes-v623-4";'));
