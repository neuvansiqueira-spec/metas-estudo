...
const previousPinned = globalThis.AldusDuplicateDiagnosticsV309;
const previousCompatibility = globalThis.AldusDuplicateDiagnosticsV260;

try {
  globalThis.AldusDuplicateDiagnosticsV309 = diagnostics;
  ...
  delete globalThis.AldusDuplicateDiagnosticsV309;
  assert.equal(
    batch.diagnosticsApi(),
    null,
  ...
  if (previousPinned === undefined) delete globalThis.AldusDuplicateDiagnosticsV309;
  else globalThis.AldusDuplicateDiagnosticsV309 = previousPinned;
...
assert.match(coreSource, /globalThis\.AldusDuplicateDiagnosticsV309 = API/, "o núcleo correto deve publicar um identificador imutável por geração");
assert.match(batchSource, /globalThis\.AldusDuplicateDiagnosticsV309/, "o lote deve consultar primeiro o núcleo fixado");
...
assert.match(bootstrapSource, /modernDiagnostics\?\.VERSION === "20260811-duplicate-flow-owner-v309"/, "o bootstrap deve dispensar a injeção do V260 quando o núcleo correto já estiver ativo");
