...
test("Plano do Dia usa superfícies escuras com texto legível", () => {
  assert.match(index, /aldus-daily-goals-v66\.css\?v=20260719-tempo-visibilidade-v66/);
...
test("cache, versão e cópias de publicação permanecem em paridade", () => {
  assert.equal(packageJson.version, "20260719-tempo-visibilidade-v66");
  assert.match(worker, /const CURRENT_VERSION = "20260719-tempo-visibilidade-v66"/);
