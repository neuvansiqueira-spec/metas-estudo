...
const version = '20260718-logo-exportacoes-v63';
...
test('v63 é a camada final de identidade visual das exportações', () => {
  assert.equal(JSON.parse(fs.readFileSync('package.json', 'utf8')).version, version);
  assert.match(html, new RegExp(`id="aldusExportBrandV63"[^>]+${version}`));
  assert.ok(html.indexOf('aldus-export-brand-v63.css') > html.indexOf('aldus-calendar-v62.css'));
  ...
  assert.match(serviceWorker, /"aldus-export-brand-v63\.css"/);
});

test('PDFs e imagens usam o vetor exato da nova marca Aldus', () => {
  assert.match(script, /aria-label="Logo Aldus Metas Concurso"/);
...
test('a planilha XLSX contém a logo como imagem incorporada', () => {
  ...
  assert.match(content, /Logo Aldus Metas Concurso/);
...
test('raiz e publicação GitHub Pages mantêm paridade na v63', () => {
  for (const file of ['index.html', 'script.js', 'service-worker.js', 'header-brand-fix.js', 'sync-integral-time-protection.js', 'aldus-export-brand-v63.css']) {
