(() => {
  ...
  const RELEASE_VERSION = "20260727-fabrica-polimento-visual-v164";
  ...
  function loadCurrentPatches(attempt = 0) {
    ...
    if (!coreReady) {
      if (attempt >= 2400) {
        console.error("[Aldus v164] O núcleo não ficou disponível para carregar as correções complementares.");
        return;
      }
    ...
    Promise.allSettled([
      ...
      `factory-polish-v164.js?v=${RELEASE_VERSION}`,
      "factory-polish-v164"
    )).catch((error) => {
      console.warn("[Aldus v164] Correção complementar não carregada.", error);
    });
  }
