(() => {
  const VERSION = "20260721-fabrica-visibilidade-v122";
  const MIGRATION_ID = "factoryLeiModeloTopificadoInstaladoV3";
...
ENTREGUE O WORD COMPLETO E O LINK PARA DOWNLOAD. NÃO ENTREGUE APENAS O CONTEÚDO NO CHAT, SALVO PEDIDO EXPRESSO.`;
  ...
  const applyPrompt = () => {
    ...
    try {
      ...
      if (typeof state === "object") {
        ...
        if (!state.migrations[MIGRATION_ID]) {
          ...
          if (current && current !== FALLBACK && current !== PROMPT) {
            ...
            state.factoryPromptLibraryBackups.leiBeforeV121 ||= current;
          }
      ...
      console.error("[Aldus] Falha ao aplicar o prompt Lei v121.", error);
    }
  };
  ...
  try {
    ...
    console.error("[Aldus] Falha ao proteger a geração do prompt Lei v121.", error);
  }
  ...
  window.addEventListener("load", () => {
    if ("serviceWorker" in navigator) {
      navigator.serviceWorker.register(`service-worker-v122.js?v=${encodeURIComponent(VERSION)}`, { updateViaCache: "none" })
