(() => {
  ...
  const RELEASE_VERSION = "20260727-atualizacao-instantanea-segura-v166";
  ...
  function loadCurrentPatches(attempt = 0) {
    ...
    if (!coreReady) {
      if (attempt >= 2400) {
        console.error("[Aldus v166] O núcleo não ficou disponível para carregar as correções complementares.");
        return;
      }
    ...
    Promise.allSettled([
      ...
      loadScript(`update-flow-v166.js?v=${RELEASE_VERSION}`, "update-flow-v166")
    ]).then((results) => {
      results.forEach((result) => {
        if (result.status === "rejected") console.warn("[Aldus v166] Correção complementar não carregada.", result.reason);
      });
    });
  }
