(() => {
  ...
  const VERSION = "20260810-duplicate-recommendations-v303";
  ...
  const PRIMARY_TOPIC_FIELDS = ["topic", "subject", "subtopic", "title", "name", "assunto", "tema", "subtema", "reference"];
  ...
  function normalizedPrimaryTopic(item) {
    const raw = String(firstValue(item, PRIMARY_TOPIC_FIELDS) || "").trim();
    if (!raw) return "";
    const withoutOutlineCode = raw.replace(/^\s*\d+(?:\.\d+)+(?:\s*[-–—.:)]\s*|\s+)/, "");
    const normalized = normalizeText(withoutOutlineCode);
    const codeWords = normalizeText(codeOf(item));
    if (!codeWords || !normalized.startsWith(`${codeWords} `)) return normalized;
    return normalized.slice(codeWords.length).trim();
  }

  function comparablePrimaryToken(token) {
    const value = String(token || "");
    if (value.length <= 3) return value;
    if (value.endsWith("oes") || value.endsWith("aes")) return `${value.slice(0, -3)}ao`;
    if (value.endsWith("ais")) return `${value.slice(0, -3)}al`;
    if (value.endsWith("eis") && value !== "leis") return `${value.slice(0, -3)}el`;
    if (value.endsWith("ns")) return `${value.slice(0, -2)}m`;
    if (value.endsWith("s")) return value.slice(0, -1);
    return value;
  }

  function comparablePrimaryTokens(value) {
    return textTokens(value).map(comparablePrimaryToken);
  }
  ...
  function buildProfiles(state) {
    ...
    const profiles = items.map((item, index) => {
      ...
      const primaryTopic = normalizedPrimaryTopic(item);
      const primaryTopicTokens = textTokens(primaryTopic);
      const primaryComparableTokens = comparablePrimaryTokens(primaryTopic);
      const primaryComparableTopic = primaryComparableTokens.join(" ");
      ...
      const profile = {
        ...
        topicTokens,
        normalizedPrimaryTopic: primaryTopic,
        primaryTopicTokens,
        primaryComparableTopic,
        primaryComparableTokens,
  ...
  function evaluatePair(left, right, weights) {
    ...
    const exactPrimaryTopic = Boolean(
      left.normalizedPrimaryTopic
      && left.normalizedPrimaryTopic === right.normalizedPrimaryTopic
    );
    ...
    const primaryTopicSimilarity = weightedSimilarity(left.primaryComparableTokens, right.primaryComparableTokens, weights);
    const primaryCharacterSimilarity = trigramSimilarity(left.primaryComparableTopic, right.primaryComparableTopic);
    const primaryLengthRatio = Math.min(left.normalizedPrimaryTopic.length, right.normalizedPrimaryTopic.length)
      / (Math.max(left.normalizedPrimaryTopic.length, right.normalizedPrimaryTopic.length) || 1);
    const safeExactPrimaryTopic = exactPrimaryTopic && sameDiscipline;
    const nearPrimaryTopic = Boolean(
      sameDiscipline
      && !exactPrimaryTopic
      && !hierarchy
      && primaryLengthRatio >= 0.86
      && (
        primaryCharacterSimilarity >= 0.94
        || (
          primaryCharacterSimilarity >= 0.90
          && primaryTopicSimilarity.containment >= 0.92
          && primaryTopicSimilarity.jaccard >= 0.78
        )
      )
    );
    ...
    if (!exactTopic && !safeExactPrimaryTopic && !nearPrimaryTopic && !exactReference && !sameCode && !crossCoverage && !hasSharedCoverage && !hierarchy && !topicSimilarity.shared.length) return null;
    ...
    if (safeExactPrimaryTopic) score = Math.max(score, 0.97);
    if (nearPrimaryTopic) score = Math.max(score, 0.92);
    ...
    if (exactTopic || safeExactPrimaryTopic || exactReference || hasSharedCoverage || (sameCode && topicSimilarity.containment >= 0.65)) classification = "exact";
    ...
    else if (safeExactPrimaryTopic) reasons.push("mesmo título principal normalizado");
    else if (nearPrimaryTopic) reasons.push("títulos principais quase idênticos");
    ...
    return {
      ...
      evidence: {
        exactTopic,
        exactPrimaryTopic: safeExactPrimaryTopic,
        nearPrimaryTopic,
        exactReference,
        sameCode,
        sameDiscipline,
  ...
  function recommendedBatchPlan(report, options = {}) {
    ...
    pairs.forEach((pair) => {
      ...
      if (
        (pair?.evidence?.hierarchy || pair?.evidence?.broadSpecific)
        && pair?.evidence?.exactPrimaryTopic !== true
        && pair?.evidence?.nearPrimaryTopic !== true
      ) {
        excluded.hierarchicalOrBroad += 1;
        return;
      }
  ...
  function previewRecommendedBatch() {
    ...
    const excludedSummary = [
      ...
      [plan.excluded.differentDiscipline, "disciplinas divergentes"],
      [plan.excluded.alreadyDecided, "com decisão anterior"],
      [plan.excluded.indirectConflict, "conflitos entre grupos"]
