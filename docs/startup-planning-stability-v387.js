(() => {
  ...
  const RESTORE_SCRIPT_ID = "aldusAuthorizedGoalRestoreV390";
  const RESTORE_VERSION = "20260824-restaura-meta-planejamento-gestao-publica-v390";
  ...
  function loadAuthorizedRestoreV390() {
    if (typeof document === "undefined") return false;
    if (document.getElementById(RESTORE_SCRIPT_ID)) return true;
    const script = document.createElement("script");
    script.id = RESTORE_SCRIPT_ID;
    script.src = `daily-goal-authorized-restore-v390.js?v=${encodeURIComponent(RESTORE_VERSION)}`;
    script.async = false;
    script.addEventListener("error", () => {
      script.remove();
      console.error("[Aldus V390] Falha ao carregar a recuperação autorizada da meta do Plano do Dia.");
    }, { once: true });
