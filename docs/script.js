...
function renderBackupSummary() {
  ...
  const cards = [
    ...
    ["Metas", counts.metas], ["Registros de desempenho", counts.questoes], ["Questões no treino", counts.bancoQuestoes], ["Treinos realizados", counts.treinosBanco], ["Revisão de erros", counts.cadernoErros], ["Simulados", counts.simulados], ["Materiais", counts.materiais], ["Revisões previstas", counts.revisoes], ["Registros históricos", counts.historico]
  ];
...
function renderBackupPreview(payload) {
  ...
  elements.backupPreview.innerHTML = `<h3>Pré-visualização do backup selecionado</h3><div class="stats-grid compact backup-summary"><article class="stat-card"><span>Data do backup</span><strong class="stat-value-date">${escapeHTML(backupDate)}</strong></article><article class="stat-card"><span>Disciplinas</span><strong>${counts.disciplinas}</strong></article><article class="stat-card"><span>Assuntos</span><strong>${counts.verticalizado}</strong></article><article class="stat-card"><span>Metas</span><strong>${counts.metas}</strong></article><article class="stat-card"><span>Registros de desempenho</span><strong>${counts.questoes}</strong></article><article class="stat-card"><span>Questões no treino</span><strong>${counts.bancoQuestoes}</strong></article><article class="stat-card"><span>Treinos realizados</span><strong>${counts.treinosBanco}</strong></article><article class="stat-card"><span>Revisão de erros</span><strong>${counts.cadernoErros}</strong></article><article class="stat-card"><span>Simulados</span><strong>${counts.simulados}</strong></article><article class="stat-card"><span>Revisões</span><strong>${counts.revisoes}</strong></article></div><p class="item-meta"><strong>Disciplinas encontradas:</strong> ${disciplines.slice(0, 20).map(escapeHTML).join(", ") || "nenhuma"}${disciplines.length > 20 ? "..." : ""}</p><div class="actions"><button type="button" data-backup-import="replace" class="danger">Substituir dados atuais</button><button type="button" data-backup-import="merge" class="secondary-button">Mesclar com dados atuais</button><button type="button" data-backup-import="cancel">Cancelar</button></div>`;
  return normalized;
}
...
function restoreBackup(payload, mode) {
  ...
  persistAndRender(); showView("backup"); elements.backupPreview.innerHTML += `<p class="notice">Backup ${mode === "replace" ? "substituído" : "mesclado"} com sucesso.</p>`;
}
...
function clearAllLocalDataFromBackup() {
  ...
  clearProjectLocalStorage(); replaceState({}); persistAndRender(); showView("backup");
}
...
function setItemStatus(id, status) { const item = state.syllabusItems.find((entry) => entry.id === id); if (item) { item.status = status; persistAndRender(); } }
function setItemDomain(id, domain, manualWeak = false) { const item = state.syllabusItems.find((entry) => entry.id === id); if (item) { item.domain = domain; item.manualWeak = manualWeak || item.manualWeak; persistAndRender(); } }
function toggleItemSchedulable(id) { const setting = settingFor(id); setting.availability = setting.availability === "Agendável" ? "Não agendável" : "Agendável"; persistAndRender(); }
...
function qbRenderErrorNotebook() { if (!elements.qbErrorStats) return; qbRenderNotebookFilters(); const stats=qbNotebookStats(), all=qbFilteredNotebook(), list=all.slice(0, QB_RENDER_LIMIT), hidden=Math.max(0, all.length - list.length); elements.qbErrorStats.innerHTML = [
    qbNotebookStatCard("Ocorrências de erro", stats.total, "Soma das ocorrências registradas na Revisão de Erros"),
  ...
  ].join(""); if (elements.qbErrorNotebookList) elements.qbErrorNotebookList.innerHTML = all.length ? `<p class="item-meta">Exibindo ${list.length} de ${all.length} registro(s) no histórico detalhado.${hidden ? " Ajuste os filtros para refinar." : ""}</p>` + list.map(qbHistoryRow).join("") : "Nenhum registro na Revisão de Erros."; }
function qbReviewFilteredBy(field) { const value = field === "disciplina" ? elements.qbErrorFilterDiscipline?.value : elements.qbErrorFilterSubject?.value; if (!value) return alert(`Selecione ${field === "disciplina" ? "uma disciplina" : "um assunto"} nos filtros simples da Revisão de Erros.`); qbStartNotebookTraining(qbFilteredNotebook().filter((item)=>item.status!=="dominado" && item[field]===value)); }
...
function qbStartNotebookTraining(items = qbFilteredNotebook().filter(i=>i.status!=="dominado")) { const questions = items.map(qbNotebookQuestionForItem).filter(Boolean); if (!questions.length) return alert("Nenhuma questão disponível para revisão. Verifique se há registros na Revisão de Erros."); qbStart(questions, { mode:"errorNotebook" }); if (typeof showView === "function") showView("banco-questoes"); }
...
function qbRenderInactiveTrainingNotice() {
  if (!elements.qbTrainingPanel || !elements.qbResultPanel || !elements.qbQuestionCard) return;
  elements.qbTrainingPanel.hidden = false;
  elements.qbResultPanel.hidden = true;
  if (elements.treinoQuestoesTitle) elements.treinoQuestoesTitle.textContent = "Treino por Questões";
  if (elements.qbTrainingCounter) elements.qbTrainingCounter.textContent = "0/0";
  if (elements.qbTrainingProgress) elements.qbTrainingProgress.style.width = "0%";
  elements.qbQuestionCard.innerHTML = `<div class="notice qb-empty-training"><strong>Nenhum treino ativo.</strong><p>Nenhum treino ativo. Inicie um treino em Treino por Questões ou Revisão de Erros.</p><div class="actions"><a class="button" href="#banco-questoes" data-view-link="banco-questoes">Ir para Treino por Questões</a><a class="secondary-button" href="#caderno-erros" data-view-link="caderno-erros">Ir para Revisão de Erros</a></div></div>`;
}
function qbRenderTrainingView() {
...
function renderActiveView() {
  if (typeof hashToView === "function") renderView(hashToView());
}
function render() {
  migrateIncorrectWeakDomains();
  renderDashboard();
  renderGoalDashboardCards();
  renderBackupSummary();
  renderActiveView();
}
...
elements.subjectForm.addEventListener("submit", (event) => { event.preventDefault(); state.subjects.push({ id: createId(), name: elements.subjectName.value.trim(), goalHours: Number(elements.subjectGoal.value) }); elements.subjectForm.reset(); persistAndRender(); });
...
elements.saveBulk.addEventListener("click", () => { state.syllabusItems.push(...bulkDraft); bulkDraft = []; elements.bulkInput.value = ""; elements.bulkPreview.innerHTML = ""; elements.saveBulk.disabled = true; persistAndRender(); });
...
elements.syllabusList.addEventListener("click", (event) => { const button = event.target.closest("button[data-action]"); if (!button) return; const { action, id } = button.dataset; if (action === "edit") editSyllabusItem(id); if (action === "not-started") setItemStatus(id, "Não iniciado"); if (action === "started") setItemStatus(id, "Em andamento"); if (action === "studied") setItemStatus(id, "Concluído"); if (action === "review") { updateItemProgress(id, { status: "Revisar", reviewed: true, lastReviewedAt: todayISO() }); persistAndRender(); } if (action === "dominated") setItemStatus(id, "Dominado"); if (action === "weak") setItemDomain(id, "Fraco", true); if (action === "schedulable") toggleItemSchedulable(id); });
elements.syllabusList.addEventListener("change", (event) => { const id = event.target.dataset.progressId; if (!id) return; const item = getSyllabusById(id); if (!item) return; if (event.target.dataset.progressField === "minutes") item.studyMinutes = Math.max(0, Number(event.target.value) || 0); if (event.target.dataset.progressField === "notes") item.progressNotes = event.target.value.trim(); persistAndRender(); });
elements.showMoreSyllabus.addEventListener("click", () => { syllabusVisibleCount += 30; renderSyllabus(); });
elements.applyBulkPriority.addEventListener("click", () => { const filtered = getFilteredItems(); if (!filtered.length) return alert("Nenhum item filtrado para alterar."); filtered.forEach((item) => { item.priority = elements.bulkPriority.value; }); persistAndRender(); });
elements.schedulableList.addEventListener("change", (event) => { const id = event.target.dataset.id; const key = event.target.dataset.setting; if (!id || !key) return; const setting = settingFor(id); setting[key] = event.target.type === "checkbox" ? event.target.checked : event.target.value; persistAndRender(); });
...
elements.clearData.addEventListener("click", () => { if (confirm("Tem certeza que deseja apagar todos os dados salvos neste navegador?")) { clearProjectLocalStorage(); replaceState({}); saveData(); persistAndRender(); } });
...
function saveGeneratedGoals(kind, made) {
  ...
  state.dailyGoals.push(...made); persistAndRender(); showView("calendario-metas");
}
...
elements.planningConfigForm?.addEventListener("submit", (event) => {
  ...
  persistAndRender(); showView("planejamento");
});
...
elements.disciplineWeightsList?.addEventListener("change", (event)=>{ const d=event.target.dataset.disciplineWeight; if(d){ state.disciplineWeights[d]=Number(event.target.value)||3; persistAndRender(); }});
[elements.monthlyTopicGoal, elements.monthlyHourGoal].filter(Boolean).forEach((el)=>el.addEventListener("change",()=>{ const key=(elements.calendarDate?.value||todayISO()).slice(0,7); state.monthlyGoals[key]={ topics:Number(elements.monthlyTopicGoal.value)||0, hours:Number(elements.monthlyHourGoal.value)||0 }; persistAndRender(); }));
elements.goalCalendarContent?.addEventListener("click", (event)=>{ const openPlan=event.target.closest("[data-open-day-plan]"); if(openPlan){ elements.goalDate.value=openPlan.dataset.openDayPlan; renderDailyGoals(); return showView("metas-do-dia"); } const b=event.target.closest("button[data-calendar-action],button[data-register-goal]"); if(!b) return; if(b.dataset.registerGoal) return fillQuestionFromGoal(b.dataset.registerGoal); const goal=state.dailyGoals.find(g=>g.id===b.dataset.id); if(!goal) return; if(b.dataset.calendarAction==="done") updateGoalDone(goal); if(b.dataset.calendarAction==="postpone") postponeGoal(goal); if(b.dataset.calendarAction==="time") registerGoalTime(goal); if(b.dataset.calendarAction==="edit") editGoal(goal); persistAndRender(); });
...
elements.dailyGoalsList.addEventListener("change", (event) => { const id = event.target.dataset.goalStatus; if (!id) return; const goal = state.dailyGoals.find((g) => g.id === id); if (goal) { goal.status = event.target.value; persistAndRender(); } });
...
elements.mockHistory?.addEventListener("click", (event) => { const view = event.target.closest("button[data-view-mock]"); const edit = event.target.closest("button[data-edit-mock]"); const duplicate = event.target.closest("button[data-duplicate-mock]"); const del = event.target.closest("button[data-delete-mock]"); if (view) { const m = state.simulados.find((x)=>x.id===view.dataset.viewMock); if (m) { state.simulados = state.simulados.filter((x)=>x.id!==m.id).concat(m); persistAndRender(); } } if (edit) editMock(edit.dataset.editMock); if (duplicate) { const m = state.simulados.find((x)=>x.id===duplicate.dataset.duplicateMock); if (m) { state.simulados.push(prepareMock({ ...cloneData(m), id:createId(), name:`${m.name} (cópia)` })); persistAndRender(); } } if (del && confirm("Excluir este simulado?")) { state.simulados = state.simulados.filter((x)=>x.id!==del.dataset.deleteMock); persistAndRender(); } });
...
function qbUpdateFeedbackModeDescription() {
  if (!elements.qbFeedbackModeDescription) return;
  elements.qbFeedbackModeDescription.textContent = elements.qbShowFeedbackImmediately?.checked
    ? "Modo estudo: você responde e vê gabarito e justificativa na hora."
    : "Modo simulado: o gabarito aparece apenas no resultado final.";
}
elements.qbShowFeedbackImmediately?.addEventListener("change", qbUpdateFeedbackModeDescription);
qbUpdateFeedbackModeDescription();
...
elements.qbClearBank?.addEventListener("click", () => { if(!confirm("Apagar banco de questões e treinos locais?")) return; state.questionBank=[]; state.questionBankSessions=[]; questionBankTraining=null; if (elements.qbQuestionCard) elements.qbQuestionCard.innerHTML=""; if (elements.qbTrainingPanel) elements.qbTrainingPanel.hidden=true; if (elements.qbResultPanel) elements.qbResultPanel.hidden=true; if (elements.qbResultSummary) elements.qbResultSummary.innerHTML=""; if (elements.qbResultDetails) elements.qbResultDetails.innerHTML=""; qbPreviewVisible=false; if (elements.qbPreviewSection) elements.qbPreviewSection.hidden = true; if (elements.qbFilteredPreview) elements.qbFilteredPreview.innerHTML="Clique em “Pré-visualizar” para listar as questões encontradas."; if (elements.qbMessage) elements.qbMessage.textContent="Banco local limpo."; saveData(); renderQuestionBank({ heavy:false }); });
...
document.addEventListener("click", (event) => { const open = event.target.closest("button[data-open-material]"); const edit = event.target.closest("button[data-edit-material]"); const del = event.target.closest("button[data-delete-material]"); if (open) openMaterial(open.dataset.openMaterial); if (edit) editMaterial(edit.dataset.editMaterial); if (del && confirm("Excluir este material?")) { state.materials = state.materials.filter((m)=>m.id!==del.dataset.deleteMaterial); persistAndRender(); } });
...
const viewAliases = { verticalizado: "edital-verticalizado", "treino-questoes": "banco-questoes" };
