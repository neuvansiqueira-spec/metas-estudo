...
const APP_VERSION = "20260718-logo-exportacoes-v63";
...
function generatedBrandLogoSvg() {
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" role="img" aria-label="Logo Aldus Metas Concurso" data-generated-logo="Aldus Metas Concurso"><defs><linearGradient id="aldusLogoNavy" x1="120" y1="70" x2="410" y2="445" gradientUnits="userSpaceOnUse"><stop stop-color="#0a2c66"/><stop offset="1" stop-color="#001c4d"/></linearGradient><linearGradient id="aldusLogoStar" x1="220" y1="235" x2="300" y2="360" gradientUnits="userSpaceOnUse"><stop stop-color="#6e95ff"/><stop offset="1" stop-color="#3f73ed"/></linearGradient></defs><rect x="8" y="8" width="496" height="496" rx="112" fill="#ffffff"/><path d="M256 42 477 458 370 398 256 176 142 398 35 458Z" fill="url(#aldusLogoNavy)"/><path d="M256 195c-16 55-46 102-73 147-22 38-12 67 23 92l50 36 50-36c35-25 45-54 23-92-27-45-57-92-73-147Z" fill="#ffffff"/><path d="m256 255 19 46 46 19-46 19-19 46-19-46-46-19 46-19Z" fill="url(#aldusLogoStar)"/></svg>`;
}
function generatedBrandMarkSvg(x = 0, y = 0, size = 128) {
  const scale = Number(size) / 512;
  return `<g data-generated-brand="Aldus Metas Concurso" transform="translate(${Number(x)} ${Number(y)}) scale(${scale})"><defs><linearGradient id="aldusBrandNavy" x1="120" y1="70" x2="410" y2="445" gradientUnits="userSpaceOnUse"><stop stop-color="#0a2c66"/><stop offset="1" stop-color="#001c4d"/></linearGradient><linearGradient id="aldusBrandStar" x1="220" y1="235" x2="300" y2="360" gradientUnits="userSpaceOnUse"><stop stop-color="#6e95ff"/><stop offset="1" stop-color="#3f73ed"/></linearGradient></defs><rect x="8" y="8" width="496" height="496" rx="112" fill="#ffffff"/><path d="M256 42 477 458 370 398 256 176 142 398 35 458Z" fill="url(#aldusBrandNavy)"/><path d="M256 195c-16 55-46 102-73 147-22 38-12 67 23 92l50 36 50-36c35-25 45-54 23-92-27-45-57-92-73-147Z" fill="#ffffff"/><path d="m256 255 19 46 46 19-46 19-19 46-19-46-46-19 46-19Z" fill="url(#aldusBrandStar)"/></g>`;
}
function generatedBrandPrintHtml() {
  return `<div class="generated-file-brand">${generatedBrandLogoSvg()}<strong>Aldus Metas Concurso</strong></div>`;
}
...
function spreadsheetWorksheetXml(rows = [], options = {}) {
  ...
  const maxColumns = Math.max(3, ...normalized.map((row) => row.length));
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><dimension ref="A1:${lastColumn}${Math.max(5, normalized.length + 4)}"/><sheetViews><sheetView workbookViewId="0"/></sheetViews><cols><col min="1" max="1" width="18" customWidth="1"/><col min="2" max="${maxColumns}" width="24" customWidth="1"/></cols><sheetData><row r="1" ht="58" customHeight="1">${spreadsheetCellXml("Aldus Metas Concurso", "B1", 1)}</row><row r="2" ht="26" customHeight="1">${spreadsheetCellXml(title, "B2", 4)}</row><row r="3">${spreadsheetCellXml(`Gerado em ${new Date(options.generatedAt || Date.now()).toLocaleString("pt-BR")}`, "B3", 5)}</row>${dataRows}</sheetData><mergeCells count="3"><mergeCell ref="B1:${lastColumn}1"/><mergeCell ref="B2:${lastColumn}2"/><mergeCell ref="B3:${lastColumn}3"/></mergeCells><drawing r:id="rId1"/></worksheet>`;
}
...
function buildBrandedXlsxArchive(rows, logoBytes, options = {}) {
  const drawing = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><xdr:wsDr xmlns:xdr="http://schemas.openxmlformats.org/drawingml/2006/spreadsheetDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><xdr:oneCellAnchor><xdr:from><xdr:col>0</xdr:col><xdr:colOff>0</xdr:colOff><xdr:row>0</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:from><xdr:ext cx="685800" cy="685800"/><xdr:pic><xdr:nvPicPr><xdr:cNvPr id="1" name="Logo Aldus Metas Concurso"/><xdr:cNvPicPr/></xdr:nvPicPr><xdr:blipFill><a:blip r:embed="rId1"/><a:stretch><a:fillRect/></a:stretch></xdr:blipFill><xdr:spPr><a:prstGeom prst="rect"><a:avLst/></a:prstGeom></xdr:spPr></xdr:pic><xdr:clientData/></xdr:oneCellAnchor></xdr:wsDr>`;
  const files = [
...
async function downloadGeneratedExcel(csv, filename, options = {}) {
  const logoBlob = await rasterizeSvgToPngBlob(generatedBrandLogoSvg(), { width: 256, height: 256 });
