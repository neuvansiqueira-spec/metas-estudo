...
function renderStrategicAnalysis() {
  ...
  const filters = { discipline: disciplineSelect?.value || "all", origin: document.getElementById("analyticsOriginSelect")?.value || "consolidated", custom };
  ...
  const stat = (label, value, hint = "") => `<article class="stat-card"><span>${escapeHTML(label)}</span><strong class="stat-value-compact">${escapeHTML(String(value ?? "Dados insuficientes"))}</strong>${hint ? `<small>${escapeHTML(hint)}</small>` : ""}</article>`;
  ...
  const qs = analysis.summary.questionsBySource || {};
  const questionSourceStats = [["Avulsas/Banco de Questões", qs.questionLogs?.total || 0], ["Sessões de estudo", qs.studySessions?.total || 0], ["Simulados", qs.mockExams?.total || 0], ["Total consolidado", analysis.summary.questionsTotal || 0]];
  const mockNoDetails = (state.simulados || []).filter((m) => !window.AnalyticsEngine.normalizeMockExamQuestionRows({ simulados: [m] }, analysis.period).some((r) => r.total > 0)).length;
  const mockDetailsWarning = mockNoDetails ? `<p class="notice warning-notice">Há ${mockNoDetails} ${mockNoDetails === 1 ? "simulado registrado" : "simulados registrados"}, mas ${mockNoDetails === 1 ? "ele não possui" : "eles não possuem"} acertos, erros e brancos suficientes para entrar na contagem de questões.</p>` : "";
  ...
  const progressRows = [
    ["Questões", maturity.components.questions.value, maturity.components.questions.target],
    ["Dias ativos", maturity.components.activeDays.value, maturity.components.activeDays.target],
    ["Sessões", maturity.components.sessions.value, maturity.components.sessions.target],
    ["Disciplinas com amostra mínima", maturity.components.disciplineCoverage.value, maturity.components.disciplineCoverage.target],
    ["Simulados", maturity.components.mockExams.value, maturity.components.mockExams.target]
  ];
  const accessibleTable = (headers, rows) => `<details class="analytics-table-details"><summary>Ver dados em tabela <span class="sr-only">Tabela textual acessível</span></summary><div class="responsive-table"><table><thead><tr>${headers.map((h)=>`<th>${escapeHTML(h)}</th>`).join("")}</tr></thead><tbody>${rows.map((r)=>`<tr>${r.map((c)=>`<td>${escapeHTML(String(c ?? "Dados insuficientes"))}</td>`).join("")}</tr>`).join("") || `<tr><td colspan="${headers.length}">Dados insuficientes</td></tr>`}</tbody></table></div></details>`;
  const svgBars = (rows, label, keys) => { const max = Math.max(1, ...rows.flatMap((r)=>keys.map((k)=>Math.abs(Number(r[k])||0)))); return `<div class="performance-svg-chart" role="img" aria-label="${escapeHTML(label)}">${rows.slice(0,31).map((r)=>`<div class="performance-day"><span>${escapeHTML(String(r.date || r.discipline || r.name || ""))}</span>${keys.map((k)=>`<i class="chart-${k}" title="${escapeHTML(k)}: ${escapeHTML(String(r[k]||0))}" style="height:${Math.max(4, Math.abs(Number(r[k])||0)/max*96)}px"></i>`).join("")}</div>`).join("") || "Dados insuficientes"}</div>`; };
  const trendText = (c) => c ? `${c.trend === "aumentou" ? "▲" : c.trend === "diminuiu" ? "▼" : "●"} ${c.trend}; atual ${c.current}; anterior ${c.previous}; diferença ${c.delta}${c.percentChange === null ? "; sem base anterior para comparação" : ` (${c.percentChange}%)`}` : "Comparação disponível apenas para 7, 30 e 90 dias.";
  const sortedDisciplines = dashboard.disciplines.slice().sort((a,b)=>b.minutes-a.minutes);
  const centralVisual = `<article class="panel analytics-full performance-central"><p class="eyebrow">Central Visual de Desempenho</p><h3>Resumo principal</h3><div class="stats-grid compact">${[
      ["Tempo estudado", dashboard.summary.timeLabel], ["Sessões", dashboard.summary.sessions], ["Dias ativos", dashboard.summary.activeDays], ["Questões", dashboard.summary.questions], ["Acertos", dashboard.summary.correct], ["Erros", dashboard.summary.wrong], ["Brancos", dashboard.summary.blank], ["Percentual de acerto", dashboard.summary.accuracyLabel, dashboard.summary.sampleMessage], ["Líquido Cebraspe", dashboard.summary.cebraspeNet], ["Simulados", dashboard.summary.mockExams], ["Metas concluídas", dashboard.summary.goalsCompleted], ["Regularidade semanal", `${dashboard.summary.weeklyRegularityPct}%`]
    ].map(([l,v,h])=>stat(l,v,h)).join("")}</div></article>
    <article class="panel analytics-full"><h3>Comparação com período anterior</h3>${list(Object.entries(dashboard.comparison || {}).map(([k,v])=>`${k}: ${trendText(v)}`), "Sem base anterior para comparação.")}</article>
    <article class="panel analytics-full"><div class="chart-panel-heading"><h3>Evolução diária</h3><button type="button" class="secondary-button chart-export-button" data-chart-export="daily" aria-label="Exportar gráfico Evolução diária">Exportar gráfico</button></div><p class="chart-legend">Legenda: minutos estudados e quantidade de questões por dia.</p>${svgBars(dashboard.daily, "Evolução diária de minutos estudados e questões", ["minutes","questions"])}${accessibleTable(["Data","Tempo estudado","Questões"], dashboard.daily.map((d)=>[formatDateBR(d.date), `${d.minutes} min`, d.questions]))}</article>
    <article class="panel"><div class="chart-panel-heading"><h3>Desempenho em questões</h3><button type="button" class="secondary-button chart-export-button" data-chart-export="questions" aria-label="Exportar gráfico Desempenho em questões">Exportar gráfico</button></div>${svgBars([{date:"Período", correct:dashboard.questions.correct, wrong:dashboard.questions.wrong, blank:dashboard.questions.blank, cebraspeNet:dashboard.questions.cebraspeNet}], "Acertos, erros, brancos e líquido", ["correct","wrong","blank","cebraspeNet"])}<div class="card-meta-grid"><span>Acertos: <strong>${dashboard.questions.correct}</strong></span><span>Erros: <strong>${dashboard.questions.wrong}</strong></span><span>Brancos: <strong>${dashboard.questions.blank}</strong></span><span>Líquido: <strong>${dashboard.questions.cebraspeNet}</strong></span></div></article>
    <article class="panel analytics-full"><div class="chart-panel-heading"><h3>Desempenho por disciplina</h3><button type="button" class="secondary-button chart-export-button" data-chart-export="disciplines" aria-label="Exportar gráfico Desempenho por disciplina">Exportar gráfico</button></div>${bars(sortedDisciplines, "minutes", "Tempo por disciplina")} ${accessibleTable(["Disciplina","Tempo","Questões","Acertos","Erros","Brancos","%","Líquido","Amostra","Confiança"], sortedDisciplines.map((d)=>[d.discipline, `${d.minutes} min`, d.questions, d.correct, d.wrong, d.blank, d.sampleSize >= 20 ? `${d.accuracyPct}%` : "Amostra em desenvolvimento", d.net, d.sampleSize, d.sampleLabel]))}</article>
    <article class="panel analytics-full"><div class="chart-panel-heading"><h3>Evolução dos simulados</h3><button type="button" class="secondary-button chart-export-button" data-chart-export="mocks" aria-label="Exportar gráfico Evolução dos simulados">Exportar gráfico</button></div>${accessibleTable(["Data","Nome","Acertos","Erros","Brancos","Líquido","%"], dashboard.mockExams.map((m)=>[formatDateBR(m.date), m.name, m.correct, m.wrong, m.blank, m.net, m.accuracyPct ? `${m.accuracyPct}%` : "Dados insuficientes"]))}</article>
    <article class="panel analytics-full"><div class="chart-panel-heading"><h3>Planejado x realizado</h3><button type="button" class="secondary-button chart-export-button" data-chart-export="planned" aria-label="Exportar gráfico Planejado x realizado">Exportar gráfico</button></div>${svgBars(dashboard.plannedVsActual, "Minutos planejados e realizados", ["plannedMinutes","actualMinutes"])}${accessibleTable(["Data","Minutos planejados","Minutos realizados","Metas planejadas","Metas concluídas"], dashboard.plannedVsActual.map((d)=>[formatDateBR(d.date), d.plannedMinutes, d.actualMinutes, d.plannedGoals, d.completedGoals]))}</article>
    <article class="panel analytics-full"><h3>Leitura automática do período</h3>${list(dashboard.insights, "Não há dados suficientes para comparar este período.")}</article>`;
  content.innerHTML = `
    <article class="panel analytics-full data-maturity-card"><div class="section-heading inline"><div><p class="eyebrow">Maturidade da análise</p><h3>${escapeHTML(maturity.label)}</h3></div><strong class="badge ${maturity.level === "high" ? "success" : maturity.level === "medium" ? "warn" : "neutral"}">${maturity.level === "high" ? "Confiança alta" : maturity.level === "medium" ? "Confiança média" : "Confiança baixa"}</strong></div><p><strong>Confiança: ${maturity.score}/100</strong></p><div class="maturity-main-progress"><span style="width:${maturity.score}%"></span></div><p>${escapeHTML(maturity.explanation)}</p><p>${escapeHTML(baselineText)}</p><div class="maturity-progress-list">${progressRows.map(([label,value,target])=>`<div class="maturity-progress-row"><span>${escapeHTML(label)}: ${value} de ${target}</span><div><i style="width:${Math.min(100, target ? value / target * 100 : 100)}%"></i></div></div>`).join("")}</div><h4>Próximos marcos</h4>${list(maturity.nextMilestones)}<div class="actions maturity-actions"><button type="button" data-view-link="dashboard" data-scroll-target="#studyForm">Registrar estudo</button><button type="button" data-view-link="questoes">Registrar questões</button><button type="button" data-view-link="simulados">Registrar simulado</button></div></article>
    ...
    <article class="panel analytics-full"><h3>Questões do período</h3><div class="stats-grid compact">${questionSourceStats.map(([l,v])=>stat(l,v)).join("")}</div>${mockDetailsWarning}</article>
    ...
    <article class="panel analytics-full"><h3>Sinais iniciais</h3>${list(analysis.preliminarySignals.map(s=>s.message), "Nenhum sinal inicial com dados ainda.")}</article>
...
function setupPerformanceExportControls(payload) {
  const button = document.getElementById('performanceExportButton'), dialog = document.getElementById('performanceExportDialog'), close = document.getElementById('performanceExportClose');
  const shareBtn = dialog?.querySelector('[data-performance-export="share"]'), note = document.getElementById('performanceExportShareNote');
  ...
  const open = () => { dialog.hidden = false; close?.focus(); };
  const shut = () => { dialog.hidden = true; button?.focus(); };
  button?.addEventListener('click', open, { once: true }); close?.addEventListener('click', shut, { once: true });
  dialog?.addEventListener('keydown', (e) => { if (e.key === 'Escape') shut(); }, { once: true });
  document.querySelectorAll('[data-performance-export]').forEach((b) => b.addEventListener('click', async () => {
    if (button?.disabled) return; button.disabled = true; setPerformanceExportStatus('Preparando arquivo…');
    try {
      const filenameBase = `desempenho-${payload.filters?.discipline === 'all' ? 'geral' : payload.filters?.discipline || 'geral'}-${todayISO()}`;
      if (b.dataset.performanceExport === 'pdf') { setPerformanceExportStatus('No iPhone, use Compartilhar na tela de impressão e escolha Salvar em Arquivos.'); openPerformancePrintView(payload); }
      if (b.dataset.performanceExport === 'csv') downloadGeneratedFile(new Blob([buildPerformanceCsv(payload)], { type: 'text/csv;charset=utf-8' }), `${filenameBase}.csv`);
      if (b.dataset.performanceExport === 'png' || b.dataset.performanceExport === 'share') {
        ...
        if (b.dataset.performanceExport === 'share') await shareGeneratedFile(blob, `${sanitizeExportFilename(filenameBase)}.png`, 'image/png'); else downloadGeneratedFile(blob, `${filenameBase}.png`);
      }
      if (b.dataset.performanceExport === 'charts') document.querySelector('.chart-export-button')?.scrollIntoView({ behavior: 'smooth', block: 'center' });
      setPerformanceExportStatus('Arquivo preparado.'); shut();
  ...
  document.querySelectorAll('[data-chart-export]').forEach((b) => b.addEventListener('click', async () => {
    if (b.disabled) return; b.disabled = true; setPerformanceExportStatus('Preparando arquivo…');
    try { const type = b.dataset.chartExport; const title = b.getAttribute('aria-label').replace('Exportar gráfico ', ''); const rows = chartRowsForType(type, payload); const meta = { title, period: payload.filters?.mode || 'Período selecionado', discipline: payload.filters?.discipline, origin: payload.filters?.origin, generatedAt: payload.generatedAt, filename: `grafico-${sanitizeExportFilename(title)}` }; const svg = buildChartSvg(type, rows, meta); downloadGeneratedFile(new Blob([svg], { type: 'image/svg+xml;charset=utf-8' }), `${meta.filename}.svg`); await exportChartToPng({ type, data: rows }, meta); downloadGeneratedFile(new Blob([buildPerformanceCsv({ ...payload, daily: type === 'daily' ? rows : [], disciplines: ['disciplines','hours','net'].includes(type) ? rows : [], mockExams: type === 'mocks' ? rows : [], plannedVsActual: type === 'planned' ? rows : [], questions: type === 'questions' ? (rows[0] || {}) : {} })], { type: 'text/csv;charset=utf-8' }), `${meta.filename}.csv`); setPerformanceExportStatus('Arquivo preparado.'); } catch (err) { console.error(err); setPerformanceExportStatus('Não foi possível gerar o arquivo neste dispositivo. Tente exportar apenas um gráfico ou usar o relatório para impressão.'); } finally { b.disabled = false; }
  }));
}
