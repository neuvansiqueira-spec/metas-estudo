...
function materialLinkedToSyllabus(material = {}) {
  const ids = [material.syllabusItemId, ...(material.syllabusItemIds || [])].filter(Boolean);
  if (ids.length && ids.some((id) => (state.syllabusItems || []).some((item) => item.id === id))) return true;
  if (material.subjectId && (state.subjects || []).some((subject) => subject.id === material.subjectId)) return true;
  const discipline = canonical(material.discipline || material.disciplina || "");
  const subject = canonical(material.subject || material.assunto || material.topic || "");
  if (!discipline || !subject) return false;
  return (state.syllabusItems || []).some((item) => canonical(item.discipline) === discipline && canonical(item.subject) === subject);
}
function materialStudySituation(material = {}) {
  if (!materialLinkedToSyllabus(material)) return "unlinked";
  const normalized = normalizeMaterialEstimateFields(material);
  const stats = plannedStudyStatsForMaterial(normalized);
  const estimated = validEstimatedMinutes(normalized.estimatedMinutes);
  const explicitlyDone = [normalized.status, normalized.situation, normalized.estado].some((value) => ["concluido", "concluida", "completed"].includes(canonical(value || "")));
  if ((estimated && stats.done >= estimated) || explicitlyDone) return "completed";
  if (!estimated) return "needsConfiguration";
  if (stats.done > 0 && stats.remaining > 0) return "studying";
  return "ready";
}
const MATERIAL_SITUATION_LABELS = { unlinked: "Sem vínculo", needsConfiguration: "A configurar", ready: "Prontos para estudar", studying: "Em estudo", completed: "Concluídos" };
const MATERIAL_SITUATION_ORDER = ["needsConfiguration", "ready", "studying", "completed", "unlinked"];
let expandedMaterialCardId = "";
const openedMaterialGroups = new Set(MATERIAL_SITUATION_ORDER);
function futureStudyGoalsForMaterial(material = {}) { return (state.dailyGoals || []).filter((g) => g.date >= todayISO() && g.type === "Estudo novo" && (g.estimateSourceId === material.id || (g.syllabusItemId && (g.syllabusItemId === material.syllabusItemId || (material.syllabusItemIds || []).includes(g.syllabusItemId))))); }
function materialProgressInfo(material = {}) { const m = normalizeMaterialEstimateFields(material); const stats = plannedStudyStatsForMaterial(m); const estimatedMinutes = validEstimatedMinutes(m.estimatedMinutes); const doneMinutes = Number(stats.done) || 0; const remainingMinutes = Math.max(0, estimatedMinutes - doneMinutes); const progressPercent = estimatedMinutes > 0 ? Math.min(100, Math.round(doneMinutes / estimatedMinutes * 100)) : 0; return { estimatedMinutes, doneMinutes, plannedMinutes: stats.planned || 0, remainingMinutes, progressPercent, futureGoals: futureStudyGoalsForMaterial(m).length, stats }; }
function materialActionReason(material = {}) { const situation = materialStudySituation(material); const info = materialProgressInfo(material); if (situation === "unlinked") return "Vincular ao item do edital"; if (situation === "needsConfiguration" && !info.estimatedMinutes) return "Definir carga horária"; if (situation === "needsConfiguration") return "Corrigir dados do material"; if (info.estimatedMinutes && info.remainingMinutes > 0 && !info.futureGoals) return "Gerar metas de estudo"; return ""; }
function renderMaterialFilters() {
  if (!elements.materialFilterDiscipline) return;
  const setOptions = (el, label, values) => { const current = el.value; const opts = [...new Set(values.filter(Boolean))].sort((a,b)=>a.localeCompare(b, "pt-BR")); el.innerHTML = `<option value="">${label}</option>` + opts.map((v)=>`<option ${v===current?"selected":""}>${escapeHTML(v)}</option>`).join(""); };
...
function materialMatchesFilters(m) {
  const term = canonical(elements.materialFilterText?.value || "");
  const situation = materialStudySituation(m);
  const estimate = elements.materialFilterEstimate?.value || "";
  const normalized = normalizeMaterialEstimateFields(m);
  const hasEstimate = Boolean(validEstimatedMinutes(normalized.estimatedMinutes));
  return (!elements.materialFilterDiscipline.value || m.discipline === elements.materialFilterDiscipline.value) && (!elements.materialFilterSubject.value || m.subject === elements.materialFilterSubject.value) && (!elements.materialFilterType.value || m.type === elements.materialFilterType.value) && (!elements.materialFilterOrigin.value || m.origin === elements.materialFilterOrigin.value) && (!elements.materialFilterSituation?.value || situation === elements.materialFilterSituation.value) && (!estimate || (estimate === "with" && hasEstimate) || (estimate === "without" && !hasEstimate) || (estimate === "automatic" && normalized.estimateMode === "automatic") || (estimate === "manual" && normalized.estimateMode === "manual")) && (!term || [m.title,m.name,m.discipline,m.subject,m.type,m.origin].map(canonical).join(" ").includes(term));
}
function filteredMaterials() { return state.materials.filter(materialMatchesFilters); }
function sortMaterialsList(list) {
  const sort = elements.materialSort?.value || "topic";
  const alpha = (a,b) => [a.discipline,a.subject,a.type,a.title].map((x)=>x||"").join("|").localeCompare([b.discipline,b.subject,b.type,b.title].map((x)=>x||"").join("|"), "pt-BR");
  return [...list].sort((a,b) => { const ai=materialProgressInfo(a), bi=materialProgressInfo(b); if (sort === "updated") return (b.updatedAt || b.estimatedAt || b.date || "").localeCompare(a.updatedAt || a.estimatedAt || a.date || "") || alpha(a,b); if (sort === "largestEstimate") return bi.estimatedMinutes - ai.estimatedMinutes || alpha(a,b); if (sort === "largestRemaining") return bi.remainingMinutes - ai.remainingMinutes || alpha(a,b); if (sort === "largestProgress") return bi.progressPercent - ai.progressPercent || alpha(a,b); if (sort === "smallestProgress") return ai.progressPercent - bi.progressPercent || alpha(a,b); return alpha(a,b); });
}
...
function materialCardHTML(m) {
  ...
  const situation = materialStudySituation(m); const info = materialProgressInfo(m); const expanded = expandedMaterialCardId === m.id;
  const factoryBadge = m.source === "factory" ? `<span class="badge success">Gerado pela Fábrica</span>` : "";
  const factoryButton = m.source === "factory" && m.factoryItemId ? `<button type="button" class="secondary-button" data-open-factory-item="${m.factoryItemId}">Abrir na Fábrica de Resumos</button>` : "";
  return `<article id="material-card-${escapeHTML(m.id)}" class="syllabus-card material-card compact-material-card" data-material-card="${escapeHTML(m.id)}" data-material-situation="${situation}"><header><div><p class="eyebrow">${escapeHTML(m.discipline || "Sem disciplina")}</p><h3>${escapeHTML(m.title || "Material sem título")}</h3><div class="item-meta">${escapeHTML(m.subject || "Sem assunto")} • ${escapeHTML(modulo)} • ${escapeHTML(origem)}</div></div><span class="badge neutral">${escapeHTML(MATERIAL_SITUATION_LABELS[situation])}</span></header><div class="material-badges">${factoryBadge}</div><div class="card-meta-grid material-compact-grid"><span>Tipo: ${escapeHTML(m.type || "-")}</span><span>Origem: ${escapeHTML(origem)}</span><span>Carga: ${info.estimatedMinutes ? formatHours(info.estimatedMinutes) : "sem estimativa"}</span><span>Estudado: ${formatHours(info.doneMinutes)}</span><span>Restante: ${formatHours(info.remainingMinutes)}</span><span>Metas futuras: ${info.futureGoals || 0}</span></div><div class="material-progress-line"><span>Progresso: ${info.progressPercent}%</span><div class="progress" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${info.progressPercent}"><span style="width:${info.progressPercent}%"></span></div></div><div class="card-actions"><button type="button" data-toggle-material-details="${m.id}" aria-expanded="${expanded}">${expanded ? "Fechar detalhes" : "Abrir detalhes"}</button>${factoryButton}</div><div class="material-details" ${expanded ? "" : "hidden"}>${materialEstimateSummaryHTML(m)}${materialEstimateFormHTML(m)}<div class="card-meta-grid"><span>Título: ${escapeHTML(m.title)}</span><span>Disciplina: ${escapeHTML(m.discipline)}</span><span>Assunto: ${escapeHTML(m.subject)}</span><span>Módulo: ${escapeHTML(modulo)}</span><span>Formato: ${escapeHTML(m.factoryFormat || m.type || "-")}</span><span>Origem: ${escapeHTML(origem)}</span><span>Data: ${formatDateBR(m.date)}</span><span>Link/localização: ${escapeHTML(m.link || "-")}</span></div><p class="item-meta">${escapeHTML(m.notes || "")}</p><div class="card-actions"><button type="button" data-open-material="${m.id}">Abrir material</button><button type="button" data-use-material-study="${m.id}">Usar no estudo</button><button type="button" data-edit-material="${m.id}">Editar</button><button class="danger" type="button" data-delete-material="${m.id}">Excluir</button></div></div></article>`;
}
function materialSummaryHTML(list) { const counts = { total: list.length, needsConfiguration: 0, ready: 0, studying: 0, completed: 0, unlinked: 0 }; list.forEach((m)=>counts[materialStudySituation(m)]++); return `<section class="materials-summary stats-grid compact"><article class="stat-card"><span>Total de materiais</span><strong>${counts.total}</strong></article>${MATERIAL_SITUATION_ORDER.map((key)=>`<article class="stat-card"><span>${escapeHTML(MATERIAL_SITUATION_LABELS[key])}</span><strong>${counts[key]}</strong></article>`).join("")}</section>`; }
function materialDoNowHTML(list) { const actions = list.map((m)=>({ material:m, reason:materialActionReason(m) })).filter((x)=>x.reason); if (!actions.length) return ""; return `<section class="materials-do-now"><h3>FAÇA AGORA</h3><div class="cards-list">${actions.map(({material, reason})=>`<article class="material-shortcut"><strong>${escapeHTML(reason)}</strong><span>${escapeHTML(material.discipline || "Sem disciplina")} • ${escapeHTML(material.subject || "Sem assunto")} • ${escapeHTML(material.title || "Material")}</span><button type="button" data-open-material-card="${escapeHTML(material.id)}">Abrir material</button></article>`).join("")}</div></section>`; }
function materialGroupsHTML(list) { const grouped = {}; for (const m of list) { const situation = materialStudySituation(m); (grouped[situation] ||= []).push(m); } return MATERIAL_SITUATION_ORDER.filter((key)=>grouped[key]?.length).map((key)=>`<details class="material-situation-group" data-material-group="${key}" ${openedMaterialGroups.has(key) ? "open" : ""}><summary>${escapeHTML(MATERIAL_SITUATION_LABELS[key]).toUpperCase()} — ${grouped[key].length}</summary><div class="cards-list">${sortMaterialsList(grouped[key]).map(materialCardHTML).join("")}</div></details>`).join(""); }
function renderMaterials() {
  if (!elements.materialsList) return;
  renderMaterialSelectors(); renderMaterialFilters();
  const list = sortMaterialsList(filteredMaterials());
  if (!state.materials.length) { elements.materialsList.innerHTML = `<p class="empty-message">Nenhum material cadastrado.<br>Adicione um material manualmente ou conclua um material na Fábrica de Resumos.</p>`; return; }
  if (!list.length) { elements.materialsList.innerHTML = `${materialSummaryHTML(list)}<p class="empty-message">Nenhum material corresponde aos filtros selecionados.</p>`; return; }
  elements.materialsList.innerHTML = `${materialSummaryHTML(list)}${materialDoNowHTML(list)}${materialGroupsHTML(list)}`;
}
...
[elements.materialFilterDiscipline, elements.materialFilterSubject, elements.materialFilterType, elements.materialFilterOrigin, elements.materialFilterText, elements.materialFilterSituation, elements.materialFilterEstimate, elements.materialSort].filter(Boolean).forEach((filter) => filter.addEventListener("input", renderMaterials));
[elements.materialFilterDiscipline, elements.materialFilterSubject, elements.materialFilterType, elements.materialFilterOrigin, elements.materialFilterSituation, elements.materialFilterEstimate, elements.materialSort].filter(Boolean).forEach((filter) => filter.addEventListener("change", renderMaterials));
elements.clearMaterialFilters?.addEventListener("click", () => { [elements.materialFilterDiscipline, elements.materialFilterSubject, elements.materialFilterType, elements.materialFilterOrigin, elements.materialFilterText, elements.materialFilterSituation, elements.materialFilterEstimate].filter(Boolean).forEach((el)=>{ el.value = ""; }); if (elements.materialSort) elements.materialSort.value = "topic"; renderMaterials(); });
document.addEventListener("change", (event) => { const group = event.target.closest?.("details[data-material-group]"); if (group) { if (group.open) openedMaterialGroups.add(group.dataset.materialGroup); else openedMaterialGroups.delete(group.dataset.materialGroup); } const mode = event.target.closest?.('[data-material-estimate-field="estimateMode"]'); if (mode) updateMaterialEstimateModeUI(mode.closest(".material-estimate-box")); });
document.addEventListener("click", (event) => { const openUrl = event.target.closest("button[data-open-url]"); if (openUrl && isValidHttpUrl(openUrl.dataset.openUrl)) window.open(openUrl.dataset.openUrl, "_blank", "noopener"); const toggleDetails = event.target.closest("button[data-toggle-material-details]"); const openCard = event.target.closest("button[data-open-material-card]"); const openFactory = event.target.closest("button[data-open-factory-item]"); const open = event.target.closest("button[data-open-material]"); const create = event.target.closest("button[data-create-goal-material]"); const edit = event.target.closest("button[data-edit-material]"); const del = event.target.closest("button[data-delete-material]"); const calcEstimate = event.target.closest("button[data-calculate-material-estimate]"); const saveEstimate = event.target.closest("button[data-save-material-estimate]"); const updateMaterialGoals = event.target.closest("button[data-update-material-goals]"); if (toggleDetails) { event.preventDefault(); expandedMaterialCardId = expandedMaterialCardId === toggleDetails.dataset.toggleMaterialDetails ? "" : toggleDetails.dataset.toggleMaterialDetails; renderMaterials(); } if (openCard) { event.preventDefault(); const material = state.materials.find((m)=>m.id===openCard.dataset.openMaterialCard); if (material) openedMaterialGroups.add(materialStudySituation(material)); expandedMaterialCardId = openCard.dataset.openMaterialCard; renderMaterials(); setTimeout(()=>document.getElementById(`material-card-${expandedMaterialCardId}`)?.scrollIntoView({ behavior: "smooth", block: "center" }), 0); } if (openFactory) { event.preventDefault(); showView("fabrica-resumos"); factoryCurrentFilter = "todos"; renderFactory(); setTimeout(()=>document.querySelector(`[data-factory-card="${CSS.escape(openFactory.dataset.openFactoryItem)}"]`)?.scrollIntoView({ behavior: "smooth", block: "center" }), 0); } if (updateMaterialGoals) { event.preventDefault(); expandedMaterialCardId = updateMaterialGoals.dataset.updateMaterialGoals; updateFuturePendingGoalsForMaterial(updateMaterialGoals.dataset.updateMaterialGoals); } if (calcEstimate) { event.preventDefault(); previewMaterialEstimate(calcEstimate); } if (saveEstimate) { event.preventDefault(); expandedMaterialCardId = saveEstimate.dataset.saveMaterialEstimate; saveMaterialEstimate(saveEstimate); } if (open) openMaterial(open.dataset.openMaterial); if (create) startMaterialForGoal(create.dataset.discipline, create.dataset.subject); if (edit) editMaterial(edit.dataset.editMaterial); if (del && confirm("Excluir este material?")) { state.materials = state.materials.filter((m)=>m.id!==del.dataset.deleteMaterial); render(); } });
