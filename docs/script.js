...
const APP_VERSION = "20260716-calendario-separado-v10";
...
function buildScopedGoalCalendarSvg(payload, scope = "all") {
  ...
  const H=Math.max(700,y+100); return `<?xml version="1.0" encoding="UTF-8"?><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${W} ${H}" role="img"><title>${title}</title><defs><linearGradient id="scopeHeader" x1="0" x2="1"><stop stop-color="#172554"/><stop offset=".55" stop-color="#1d4ed8"/><stop offset="1" stop-color="#0891b2"/></linearGradient></defs><style>text{font-family:Arial,Helvetica,sans-serif}.title{font-size:48px;font-weight:900;fill:#fff}.meta{font-size:22px;fill:#dbeafe}.label{font-size:20px;font-weight:800;fill:#0f172a}.hint{font-size:17px;fill:#64748b}.day{font-size:28px;font-weight:900;fill:#0e7490}</style><rect width="100%" height="100%" fill="#f8fafc"/><rect width="100%" height="210" fill="url(#scopeHeader)"/><text x="${margin}" y="78" class="title">${title}</text><text x="${margin}" y="124" class="meta">${formatDateBR(period.start)} a ${formatDateBR(period.end)} • ${period.goals.length} meta(s)</text><text x="${margin}" y="163" class="meta">${formatHours(period.planned)} planejadas • ${period.completed} concluídas • ${period.percent}%</text>${body}<text x="${margin}" y="${H-36}" class="hint">Gerado em ${escapeHTML(new Date(payload.generatedAt).toLocaleString("pt-BR"))}</text></svg>`;
}
function buildGoalCalendarPrintHTML(payload, scope = "all") {
  ...
  return `<article id="goalCalendarPrintableReport" class="calendar-print-report"><header><p>PLANEJAMENTO DE ESTUDOS</p><h1>Calendário didático de metas</h1><div>Período: <strong>${escapeHTML(goalCalendarScopeLabel(scope))}</strong> • Referência: <strong>${formatDateBR(payload.referenceDate)}</strong></div></header>${sections}<footer>Gerado em ${escapeHTML(new Date(payload.generatedAt).toLocaleString("pt-BR"))}</footer></article>`;
}
...
elements.generateDayGoals?.addEventListener("click", generateDayGoals);
