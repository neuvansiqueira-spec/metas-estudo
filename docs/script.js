...
if (typeof module !== 'undefined') module.exports = { formatExportDuration, sanitizeExportFilename, buildPerformanceExportPayload, buildPerformanceCsv, buildChartSvg };
...
function renderStrategicAnalysis() {
  ...
  content.innerHTML = `
    ...
    <article class="panel analytics-full"><h3>Gráficos</h3><div class="chart-panel-heading"><h4>Horas por disciplina</h4><button type="button" class="secondary-button chart-export-button" data-chart-export="hours" aria-label="Exportar gráfico Horas por disciplina">Exportar gráfico</button></div>${bars(analysis.disciplines, "hours", "Horas por disciplina")}<div class="chart-panel-heading"><h4>Líquido por disciplina</h4><button type="button" class="secondary-button chart-export-button" data-chart-export="net" aria-label="Exportar gráfico Líquido por disciplina">Exportar gráfico</button></div>${bars(analysis.disciplines, "net", "Líquido por disciplina")}</article>
...
function setupPerformanceExportControls(payload) {
  ...
  dialog?.querySelectorAll('[data-performance-export]').forEach((b) => b.addEventListener('click', async () => {
    ...
    try {
      ...
      if (b.dataset.performanceExport === 'pdf') openPerformancePrintView(payload);
