...
function renderSupportTable(headers, rows, label = 'Ver dados em tabela') { return `<details class="analytics-table-details didactic-table"><summary>${escapeHTML(label)} <span class="sr-only">Tabela textual acessível</span></summary><div class="responsive-table"><table><thead><tr>${headers.map((h)=>`<th>${escapeHTML(h)}</th>`).join('')}</tr></thead><tbody>${rows.length ? rows.map((r)=>`<tr>${r.map((c)=>`<td>${escapeHTML(String(c ?? ''))}</td>`).join('')}</tr>`).join('') : `<tr><td colspan="${headers.length}">Sem dados para exibir.</td></tr>`}</tbody></table></div></details>`; }
...
function renderNetByDisciplineChart(rows = [], period = {}) {
  ...
  return `<section class="didactic-chart didactic-net-chart" aria-label="Líquido por disciplina"><p class="chart-explainer">Foram registradas questões em ${withQuestions.length} ${withQuestions.length === 1 ? 'disciplina' : 'disciplinas'}. ${explanation}</p><p class="chart-legend">Escala: centro = zero; máximo absoluto = ${maxAbs} ponto(s). Unidade: líquido Cebraspe (acertos - erros) no ${escapeHTML(analyticsPeriodLabel(period))}.</p><div class="net-axis" aria-hidden="true"><span>Negativo</span><b>0</b><span>Positivo</span></div><div class="didactic-net-list">${withQuestions.map((r)=>{ const net=Number(r.net)||0, sample=Number(r.questions)||0, side=net>=0?'positive':'negative', width=Math.max(4, Math.abs(net)/maxAbs*50); return `<article class="net-discipline-row ${side}"><strong>${escapeHTML(r.discipline)}</strong><div class="net-bar-line"><span class="net-left">${net<0?`<i style="width:${width}%"></i>`:''}</span><b class="net-zero"></b><span class="net-right">${net>=0?`<i style="width:${width}%"></i>`:''}</span></div><p>Líquido: <strong>${net>0?'+':''}${net}</strong></p><small>${Number(r.correct)||0} acertos • ${Number(r.wrong)||0} erros • ${Number(r.blank)||0} brancos</small><small>Amostra: ${sample} ${sample === 1 ? 'questão' : 'questões'}</small></article>`; }).join('')}</div>${renderSupportTable(['Disciplina','Acertos','Erros','Brancos','Líquido','Amostra'], withQuestions.map((r)=>[r.discipline, r.correct, r.wrong, r.blank, r.net, r.questions]))}</section>`;
}
...
function analyticsStatCard(label, value, hint = "") {
  return `<article class="stat-card"><span>${escapeHTML(label)}</span><strong class="stat-value-compact">${escapeHTML(String(value ?? "Dados insuficientes"))}</strong>${hint ? `<small>${escapeHTML(hint)}</small>` : ""}</article>`;
}
function analyticsList(items, empty = "Nenhum item identificado.") {
  return items?.length ? `<ul>${items.slice(0, 12).map((item) => `<li>${escapeHTML(String(item))}</li>`).join("")}</ul>` : `<p class="empty-message">${escapeHTML(empty)}</p>`;
}
function renderAnalyticsHeader(viewModel) {
  return `<article class="panel analytics-full performance-central" data-analytics-environment="single"><p class="eyebrow">Central de Desempenho</p><h3>Análise Estratégica unificada</h3><p>Período: <strong>${escapeHTML(viewModel.periodLabel)}</strong> • Disciplina: <strong>${escapeHTML(viewModel.filters.discipline === "all" ? "Todas" : viewModel.filters.discipline)}</strong> • Origem: <strong>${escapeHTML(viewModel.filters.origin)}</strong></p></article>`;
}
function renderAnalyticsMaturity({ maturity, baselineText }) {
  const rows = [["Questões", maturity.components.questions.value, maturity.components.questions.target], ["Dias ativos", maturity.components.activeDays.value, maturity.components.activeDays.target], ["Sessões", maturity.components.sessions.value, maturity.components.sessions.target], ["Disciplinas com amostra mínima", maturity.components.disciplineCoverage.value, maturity.components.disciplineCoverage.target], ["Simulados", maturity.components.mockExams.value, maturity.components.mockExams.target]];
  const next = maturity.nextMilestones?.[0] || "Continue registrando estudos, questões e simulados para ampliar a confiança.";
  return `<article class="panel analytics-full data-maturity-card"><div class="section-heading inline"><div><p class="eyebrow">Maturidade dos dados</p><h3>${escapeHTML(maturity.label)}</h3></div><strong class="badge ${maturity.level === "high" ? "success" : maturity.level === "medium" ? "warn" : "neutral"}">${maturity.level === "high" ? "Confiança alta" : maturity.level === "medium" ? "Confiança média" : "Confiança baixa"}</strong></div><p><strong>Confiança: ${maturity.score}/100</strong></p><div class="maturity-main-progress"><span style="width:${Math.min(100, maturity.score)}%"></span></div><p>${escapeHTML(maturity.explanation)}</p><p><strong>Próximo marco:</strong> ${escapeHTML(next)}</p><details class="analytics-table-details"><summary>Ver detalhes da maturidade</summary><p>${escapeHTML(baselineText)}</p><div class="maturity-progress-list">${rows.map(([label,value,target])=>`<div class="maturity-progress-row"><span>${escapeHTML(label)}: ${value} de ${target}</span><div><i style="width:${Math.min(100, target ? value / target * 100 : 100)}%"></i></div></div>`).join("")}</div></details><div class="actions maturity-actions"><button type="button" data-view-link="dashboard" data-scroll-target="#studyForm">Registrar estudo</button><button type="button" data-view-link="questoes">Registrar questões</button><button type="button" data-view-link="simulados">Registrar simulado</button></div></article>`;
}
function renderAnalyticsSummary(dashboard) {
  const items = [["Tempo estudado", dashboard.summary.timeLabel], ["Sessões", dashboard.summary.sessions], ["Dias ativos", dashboard.summary.activeDays], ["Questões", dashboard.summary.questions], ["Acertos", dashboard.summary.correct], ["Erros", dashboard.summary.wrong], ["Brancos", dashboard.summary.blank], ["Percentual", dashboard.summary.accuracyLabel, dashboard.summary.sampleMessage], ["Líquido Cebraspe", dashboard.summary.cebraspeNet], ["Simulados", dashboard.summary.mockExams], ["Metas concluídas", dashboard.summary.goalsCompleted], ["Regularidade", `${dashboard.summary.weeklyRegularityPct}%`]];
  return `<article class="panel analytics-full"><h3>Resumo principal</h3><div class="stats-grid compact">${items.map(([l,v,h])=>analyticsStatCard(l,v,h)).join("")}</div></article>`;
}
function renderAnalyticsComparison(comparison = {}) {
  const labels = { hours: "Tempo estudado", questions: "Questões", accuracy: "Percentual", cebraspeNet: "Líquido Cebraspe", goals: "Metas" };
  const unit = (key, v) => key === "hours" ? humanDurationFromHours(v) : (key === "accuracy" ? `${v}%` : v);
  const cards = Object.entries(comparison).map(([key, c]) => `<article class="stat-card comparison-card"><span>${escapeHTML(labels[key] || key)}</span><small>Atual: <strong>${escapeHTML(String(unit(key, c.current)))}</strong></small><small>Anterior: <strong>${escapeHTML(String(unit(key, c.previous)))}</strong></small><strong>${c.trend === "aumentou" ? "▲ aumento" : c.trend === "diminuiu" ? "▼ redução" : "● estável"} de ${escapeHTML(String(unit(key, Math.abs(c.delta))))}</strong></article>`).join("");
  return `<article class="panel analytics-full"><h3>Comparação com período anterior</h3><div class="stats-grid compact">${cards || '<p class="empty-message">Comparação disponível apenas para períodos com base anterior.</p>'}</div></article>`;
}
function renderStudyEvolutionCharts(daily = []) {
  const maxMinutes = Math.max(1, ...daily.map((d)=>Number(d.minutes)||0));
  const maxQuestions = Math.max(1, ...daily.map((d)=>Number(d.questions)||0));
  const rows = daily.map((d)=>[formatDateBR(d.date), formatExportDuration(d.minutes), d.questions]);
  return `<article class="panel analytics-full"><div class="chart-panel-heading"><h3>Evolução do estudo</h3><button type="button" class="secondary-button chart-export-button" data-chart-export="daily" aria-label="Exportar gráfico Evolução do estudo">Exportar gráfico</button></div><div class="performance-chart-scroll"><section class="performance-chart"><h4>Tempo estudado por dia</h4><div class="performance-line-chart">${daily.map((d)=>`<button type="button" class="performance-line-point" style="height:${Math.max(6,(Number(d.minutes)||0)/maxMinutes*110)}px" title="${escapeHTML(formatDateBR(d.date))}: ${formatExportDuration(d.minutes)}"><span>${escapeHTML(formatDateBR(d.date).slice(0,5))}</span></button>`).join("") || '<p class="performance-chart-empty">Sem tempo registrado neste período.</p>'}</div></section><section class="performance-chart"><h4>Questões por dia</h4><div class="performance-bars">${daily.map((d)=>`<div class="performance-chart-row"><span>${escapeHTML(formatDateBR(d.date).slice(0,5))}</span><div class="performance-chart-track"><i style="height:${Math.max(4,(Number(d.questions)||0)/maxQuestions*110)}px"></i></div><b class="performance-chart-value">${Number(d.questions)||0}</b></div>`).join("") || '<p class="performance-chart-empty">Sem questões registradas neste período.</p>'}</div></section></div>${renderSupportTable(["Data","Tempo estudado","Questões"], rows)}</article>`;
}
function renderQuestionPerformanceChart(q = {}) {
  const total = (Number(q.correct)||0) + (Number(q.wrong)||0) + (Number(q.blank)||0);
  if (!total) return `<article class="panel analytics-full"><h3>Desempenho em questões</h3><p class="empty-message">Ainda não existem questões registradas neste período.</p></article>`;
  const seg = (value, cls) => `<i class="${cls}" style="width:${Math.max(0, value / total * 100)}%"></i>`;
  return `<article class="panel analytics-full"><div class="chart-panel-heading"><h3>Desempenho em questões</h3><button type="button" class="secondary-button chart-export-button" data-chart-export="questions" aria-label="Exportar gráfico Desempenho em questões">Exportar gráfico</button></div><div class="question-composition" aria-label="Composição de acertos, erros e brancos">${seg(q.correct,'correct')}${seg(q.wrong,'wrong')}${seg(q.blank,'blank')}</div><div class="card-meta-grid"><span>Acertos: <strong>${q.correct}</strong></span><span>Erros: <strong>${q.wrong}</strong></span><span>Brancos: <strong>${q.blank}</strong></span><span>Líquido em destaque: <strong>${q.cebraspeNet}</strong></span></div></article>`;
}
function renderDisciplinePerformancePanel(disciplines = [], period = {}) {
  const sorted = disciplines.slice().sort((a,b)=>(b.questions||0)-(a.questions||0));
  const cards = sorted.map((d)=>`<article class="discipline-performance-card"><h4>${escapeHTML(d.discipline)}</h4><div class="card-meta-grid"><span>Tempo: <strong>${formatExportDuration(d.minutes)}</strong></span><span>Questões: <strong>${d.questions}</strong></span><span>Percentual: <strong>${d.sampleSize >= 20 ? `${d.accuracyPct}%` : 'Amostra em desenvolvimento'}</strong></span><span>Líquido: <strong>${d.net > 0 ? '+' : ''}${d.net}</strong></span><span>Amostra: <strong>${d.sampleSize}</strong></span><span>Confiança: <strong>${escapeHTML(d.sampleLabel || 'Em formação')}</strong></span></div></article>`).join("") || '<p class="empty-message">Sem disciplinas com dados no período.</p>';
  return `<article class="panel analytics-full"><div class="chart-panel-heading"><h3>Desempenho por disciplina</h3><label class="discipline-sort-label">Ordenar <select data-discipline-sort><option>mais questões</option><option>maior tempo</option><option>maior líquido</option><option>menor líquido</option><option>maior acerto</option><option>menor acerto</option></select></label></div><div class="discipline-performance-list">${cards}</div><h4>Horas por disciplina</h4>${renderHoursByDisciplineChart(disciplines, period)}<h4>Líquido por disciplina</h4>${renderNetByDisciplineChart(disciplines, period)}</article>`;
}
function renderMockEvolutionChart(mocks = []) {
  if (!mocks.length) return `<article class="panel analytics-full"><h3>Evolução dos simulados</h3><p class="empty-message">Nenhum simulado registrado no período.</p></article>`;
  if (mocks.length === 1) { const m=mocks[0]; return `<article class="panel analytics-full"><h3>Evolução dos simulados</h3><article class="stat-card"><span>${escapeHTML(m.name || 'Simulado')}</span><strong>Líquido: ${m.net ?? 'Dados insuficientes'}</strong><small>${formatDateBR(m.date)} • ${m.accuracyPct ? `${m.accuracyPct}%` : 'percentual indisponível'}</small></article></article>`; }
  const max = Math.max(1, ...mocks.map((m)=>Math.abs(Number(m.net)||0)));
  return `<article class="panel analytics-full"><div class="chart-panel-heading"><h3>Evolução dos simulados</h3><button type="button" class="secondary-button chart-export-button" data-chart-export="mocks" aria-label="Exportar gráfico Evolução dos simulados">Exportar gráfico</button></div><div class="performance-line-chart mock-line-chart">${mocks.map((m)=>`<button type="button" class="performance-line-point" style="height:${Math.max(8, Math.abs(Number(m.net)||0)/max*120)}px" title="${escapeHTML(m.name || 'Simulado')} • ${formatDateBR(m.date)} • Líquido ${m.net} • ${m.accuracyPct || 0}%"><span>${escapeHTML(formatDateBR(m.date).slice(0,5))}</span></button>`).join("")}</div>${renderSupportTable(["Data","Nome","Líquido","Percentual"], mocks.map((m)=>[formatDateBR(m.date), m.name, m.net, m.accuracyPct ? `${m.accuracyPct}%` : 'Dados insuficientes']))}</article>`;
}
function renderPlannedVsActualChart(rows = []) {
  const max = Math.max(1, ...rows.flatMap((r)=>[r.plannedMinutes, r.actualMinutes].map(Number)));
  const totalPlanned = rows.reduce((s,r)=>s+(Number(r.plannedMinutes)||0),0), totalActual = rows.reduce((s,r)=>s+(Number(r.actualMinutes)||0),0);
  const diff = totalPlanned - totalActual;
  return `<article class="panel analytics-full"><div class="chart-panel-heading"><h3>Planejado x realizado</h3><button type="button" class="secondary-button chart-export-button" data-chart-export="planned" aria-label="Exportar gráfico Planejado x realizado">Exportar gráfico</button></div><div class="paired-bars">${rows.map((r)=>`<div class="performance-chart-row"><span>${escapeHTML(formatDateBR(r.date).slice(0,5))}</span><div class="performance-chart-track"><i class="planned" style="width:${(Number(r.plannedMinutes)||0)/max*100}%"></i><i class="actual" style="width:${(Number(r.actualMinutes)||0)/max*100}%"></i></div></div>`).join("") || '<p class="performance-chart-empty">Sem planejamento no período.</p>'}</div><p>Planejado: <strong>${formatExportDuration(totalPlanned)}</strong> • Realizado: <strong>${formatExportDuration(totalActual)}</strong> • ${diff > 0 ? 'Faltaram' : 'Sobraram'}: <strong>${formatExportDuration(Math.abs(diff))}</strong></p><p class="item-meta">Metas planejadas/concluídas aparecem na tabela de apoio, sem misturar metas e minutos na escala.</p>${renderSupportTable(["Data","Planejado","Realizado","Metas planejadas","Metas concluídas"], rows.map((r)=>[formatDateBR(r.date), formatExportDuration(r.plannedMinutes), formatExportDuration(r.actualMinutes), r.plannedGoals, r.completedGoals]))}</article>`;
}
function renderAnalyticsInsights(dashboard = {}, analysis = {}) {
  const insights = (dashboard.insights || []).slice(0, 5);
  const extra = (dashboard.insights || []).slice(5);
  return `<article class="panel analytics-full"><h3>Leitura automática</h3>${analyticsList(insights, "Não há dados suficientes para conclusões automáticas.")}${extra.length ? `<details class="analytics-table-details"><summary>Ver detalhes adicionais</summary>${analyticsList(extra)}</details>` : ""}</article>`;
}
function renderDetailedDiagnosis(analysis) {
  return `<details class="panel analytics-full"><summary>Ver diagnóstico estratégico detalhado</summary><h4>Situação geral</h4><p>${escapeHTML(analysis.overallSituation.explanation)}</p><h4>Disciplinas fortes</h4>${analyticsList((analysis.strongDisciplines||[]).map((d)=>`${d.discipline}: ${d.justification}`), "Nenhuma disciplina forte identificada.")}<h4>Disciplinas críticas</h4>${analyticsList((analysis.criticalDisciplines||[]).map((d)=>`${d.discipline}: ${d.justification}`), "Nenhuma disciplina crítica identificada.")}<h4>Sinais iniciais</h4>${analyticsList((analysis.preliminarySignals||[]).map((s)=>s.message), "Nenhum sinal inicial.")}<h4>Assuntos negligenciados</h4>${analyticsList((analysis.neglectedSubjects||[]).slice(0,20).map((s)=>`${s.discipline} — ${s.subject}: ${s.reason}`), "Nenhum assunto negligenciado identificado.")}<h4>Eficiência</h4><p>${escapeHTML(analysis.efficiency.warning || "Sem alerta de eficiência.")}</p><h4>Recomendações</h4>${analyticsList([...(analysis.recommendations.highPriority||[]), ...(analysis.recommendations.prioritySubjects||[]), ...(analysis.recommendations.maintenance||[])], "Sem recomendações automáticas.")}</details>`;
}
function renderAnalyticsQualityDetails(analysis) {
  return `<details class="panel analytics-full"><summary>Qualidade dos dados</summary>${analyticsList([`Disciplinas sem lançamentos: ${analysis.dataQuality.disciplinesWithoutEntries.join(", ") || "nenhuma"}`, `Estudos sem assunto/disciplina vinculada: ${analysis.dataQuality.studiesWithoutSubject}`, `Questões sem disciplina: ${analysis.dataQuality.questionsWithoutDiscipline}`, `Estudos sem assunto: ${analysis.dataQuality.studiesWithoutTopic}`, `Questões sem assunto: ${analysis.dataQuality.questionsWithoutSubject}`, `Lançamentos com total inconsistente: ${analysis.dataQuality.inconsistentQuestionTotals}`, `Simulados sem resultado: ${analysis.dataQuality.mockExamsWithoutResult}`, `Disciplinas com menos de 20 questões: ${analysis.dataQuality.disciplinesBelow20Questions.join(", ") || "nenhuma"}`])}</details><details class="panel analytics-full"><summary>Como a análise foi calculada</summary><p>Disciplina forte exige pelo menos 20 questões, 70% de acerto, líquido positivo e cumprimento aceitável de metas quando existirem. Disciplina crítica combina baixo percentual, líquido baixo/negativo, muitos erros, queda recente ou metas não concluídas. Negligência considera itens não concluídos/ignorados sem estudo recente, sem questões ou com metas pendentes. O líquido Cebraspe é acertos menos erros; brancos são neutros. O período atual é comparado ao período imediatamente anterior de mesma duração.</p></details>`;
}
function renderStrategicAnalysis() {
  ...
  const disciplineSelect = document.getElementById("analyticsDisciplineSelect");
  if (disciplineSelect && disciplineSelect.options.length <= 1) {
    const names = [...new Set([...(state.subjects || []).map((x) => x.name), ...(state.studies || []).map((x) => x.discipline), ...(state.questionLogs || []).map((x) => x.discipline), ...(state.dailyGoals || []).map((x) => x.discipline), ...(state.syllabusItems || []).map((x) => x.discipline)].filter(Boolean))].sort();
    disciplineSelect.insertAdjacentHTML("beforeend", names.map((name) => `<option value="${escapeHTML(name)}">${escapeHTML(name)}</option>`).join(""));
  }
  const filters = { discipline: disciplineSelect?.value || "all", origin: document.getElementById("analyticsOriginSelect")?.value || "consolidated", custom, mode, periodLabel: periodSelect?.selectedOptions?.[0]?.textContent || mode };
  const analysis = window.AnalyticsEngine.buildStrategicAnalysis(state, ["month", "all"].includes(mode) ? "30d" : mode, custom, { today: todayISO(), minStrongQuestions: 20 });
  const dashboard = window.AnalyticsEngine.buildPerformanceDashboard(state, mode, filters, { today: todayISO(), minStrongQuestions: 20 });
  const maturity = analysis.dataMaturity;
  const baselineText = maturity.baseline.firstRecordDate ? `Linha de base iniciada em ${formatDateBR(maturity.baseline.firstRecordDate)}.` : "Nenhum dado registrado para iniciar a linha de base.";
  const viewModel = { filters, periodLabel: analyticsPeriodLabel(analysis.period), dashboard, analysis, maturity, baselineText };
  content.innerHTML = [renderAnalyticsHeader(viewModel), renderAnalyticsMaturity(viewModel), renderAnalyticsSummary(dashboard), renderAnalyticsComparison(dashboard.comparison), renderStudyEvolutionCharts(dashboard.daily), renderQuestionPerformanceChart(dashboard.questions), renderDisciplinePerformancePanel(dashboard.disciplines, analysis.period), renderMockEvolutionChart(dashboard.mockExams), renderPlannedVsActualChart(dashboard.plannedVsActual), renderAnalyticsInsights(dashboard, analysis), renderDetailedDiagnosis(analysis), renderAnalyticsQualityDetails(analysis)].join("");
  setupPerformanceExportControls(buildPerformanceExportPayload(dashboard, filters, analysis));
}
...
async function handleChartExportAction(trigger) {
  const payload = currentPerformanceExportPayload();
  if (!trigger || trigger.disabled) return;
  trigger.disabled = true; setPerformanceExportStatus('Preparando arquivo…');
  try {
    const type = trigger.dataset.chartExport;
    const title = trigger.getAttribute('aria-label').replace('Exportar gráfico ', '');
    const rows = chartRowsForType(type, payload);
    const meta = { title, period: payload.filters?.mode || 'Período selecionado', discipline: payload.filters?.discipline, origin: payload.filters?.origin, generatedAt: payload.generatedAt, filename: `grafico-${sanitizeExportFilename(title)}` };
    const svg = buildChartSvg(type, rows, meta);
    downloadGeneratedFile(new Blob([svg], { type: 'image/svg+xml;charset=utf-8' }), `${meta.filename}.svg`);
    await exportChartToPng({ type, data: rows }, meta);
    downloadGeneratedFile(new Blob([buildPerformanceCsv({ ...payload, daily: type === 'daily' ? rows : [], disciplines: ['disciplines','hours','net'].includes(type) ? rows : [], mockExams: type === 'mocks' ? rows : [], plannedVsActual: type === 'planned' ? rows : [], questions: type === 'questions' ? (rows[0] || {}) : {} })], { type: 'text/csv;charset=utf-8' }), `${meta.filename}.csv`);
    setPerformanceExportStatus('Arquivo preparado.');
  } catch (err) { console.error(err); setPerformanceExportStatus('Não foi possível gerar o arquivo neste dispositivo. Tente exportar apenas um gráfico ou usar o relatório para impressão.'); }
...
function setupPerformanceExportControls(payload) {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (chartButton) { event.preventDefault(); handleChartExportAction(chartButton); }
  });
