(() => {
  ...
  function checksumText(text) {
    ...
    return `fnv1a-v304-${(hash >>> 0).toString(16).padStart(8, "0")}-${text.length}`;
  }
  ...
  async function saveBackupSnapshot(targetState, label) {
    ...
    const record = {
      id: `v304-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      version: VERSION,
      createdAt,
      label,
  ...
  function recordDiagnostic(details) {
    globalThis.__aldusDuplicateBatchV304LastRun = Object.freeze(cloneData(details));
    try {
      localStorage.setItem("aldusDuplicateBatchV304LastRun", JSON.stringify(details));
    } catch {}
  }
  ...
  function cachedPlanForState(targetState, api) {
    const cached = globalThis.__aldusDuplicateBatchPlanV304;
  ...
  async function applyBatch(event) {
    ...
    closePreview(root);
    setStatus(root, "V304: preparando a consolidação sem recalcular o diagnóstico…");
    await yieldToBrowser();

    let plan = cachedPlanForState(targetState, api);
    if (!plan) {
      setStatus(root, "V304: o estado mudou; recalculando o plano de segurança…");
      await yieldToBrowser();
      const report = api.diagnoseState(targetState, { includeDecided: true });
      plan = api.recommendedBatchPlan(report);
    }
    ...
    setStatus(root, `V304: criando cópia integral antes de ${plan.actions.length} consolidações…`);
    await yieldToBrowser();
    ...
    try {
      ...
      for (let index = 0; index < plan.actions.length; index += 1) {
        ...
        const result = api.consolidateItems(workingState, action.keeperId, action.removedId, {
          backupId: backup.id,
          decidedAt: startedAt,
          auditId: `v304-${Date.now()}-${index + 1}`
        });
        remappedLinks += Number(result?.remappedLinks) || 0;
        setStatus(root, `V304: processando ${index + 1} de ${plan.actions.length} consolidações…`);
        await yieldToBrowser();
      }
      ...
      setStatus(root, "V304: gravando e verificando o estado ativo e o IndexedDB…");
      ...
      setStatus(root, `V304 concluída: ${plan.actions.length} metas consolidadas, ${remappedLinks} vínculos preservados e gravação confirmada. Atualizando o diagnóstico…`, "success");
      await yieldToBrowser();
      ...
      setControlsBusy(root, false);
      setStatus(root, `V304 não concluiu o lote: ${String(error?.message || error)} Nenhuma exclusão foi mantida.`, "error");
    }
  }
  ...
  function markInstalled() {
    ...
    if (button) {
      ...
      button.title = "Consolidação V304 sem diagnóstico duplicado, com progresso e verificação pós-gravação.";
    }
    return Boolean(root && button);
  }

  function install() {
    if (globalThis.__aldusDuplicateBatchAuthoritativeV304) return;
    ...
    globalThis.__aldusDuplicateBatchAuthoritativeV304 = Object.freeze({
      version: VERSION,
  ...
  const API = Object.freeze({ VERSION, cloneData, replaceStateContents, removedIdsFromPlan, remainingRemovedIds, cachedPlanForState });
