...
async function aldusV207ApplicationResponse(baseResponse) {
  ...
  try {
    ...
    let patchedSource = source
      .replaceAll(ALDUS_V199_VERSION, ALDUS_V207_VERSION)
      .replaceAll(ALDUS_V204_VERSION, ALDUS_V207_VERSION)
      .replaceAll(ALDUS_V205_VERSION, ALDUS_V207_VERSION)
      .replaceAll(ALDUS_V206_VERSION, ALDUS_V207_VERSION);
    ...
    headers.set("x-aldus-runtime-patch", ALDUS_V207_VERSION);
    ...
    console.warn("[Aldus V207] Não foi possível suavizar o título da navegação; a V199 será mantida.", error);
    return baseResponse;
  }
}
...
aldusNativeAddEventListener("fetch", (event) => {
  ...
  event.respondWith(isApplication ? baseResponsePromise.then(aldusV207ApplicationResponse) : baseResponsePromise);
});