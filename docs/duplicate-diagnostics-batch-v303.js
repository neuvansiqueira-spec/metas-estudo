(() => {
  ...
  function checksumText(text) {
    ...
    return `fnv1a-v303-${(hash >>> 0).toString(16).padStart(8, "0")}-${text.length}`;
  }
  ...
  async function saveBackupSnapshot(targetState, label) {
    ...
    const record = {
      id: `v303-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
      version: VERSION,
      createdAt,
      label,
  ...
  function recordDiagnostic(details) {
    globalThis.__aldusDuplicateBatchV303LastRun = Object.freeze(cloneData(details));
    try {
      localStorage.setItem("aldusDuplicateBatchV303LastRun", JSON.stringify(details));
    } catch {}
  }
  ...
  async function applyBatch(event) {
    ...
    closePreview(root);
    setStatus(root, `V303: criando cópia integral antes de ${plan.actions.length} consolidações…`);
    ...
    try {
      ...
      for (let index = 0; index < plan.actions.length; index += 1) {
        ...
        const result = api.consolidateItems(workingState, action.keeperId, action.removedId, {
          backupId: backup.id,
          decidedAt: startedAt,
          auditId: `v303-${Date.now()}-${index + 1}`
        });
        remappedLinks += Number(result?.remappedLinks) || 0;
        if ((index + 1) % 5 === 0 || index + 1 === plan.actions.length) {
          setStatus(root, `V303: processando ${index + 1} de ${plan.actions.length} consolidações…`);
      ...
      setStatus(root, "V303: gravando e verificando o estado ativo e o IndexedDB…");
      ...
      setStatus(root, `V303 concluída: ${plan.actions.length} metas consolidadas, ${remappedLinks} vínculos preservados e gravação confirmada. Atualizando o diagnóstico…`, "success");
      ...
      setControlsBusy(root, false);
      setStatus(root, `V303 não concluiu o lote: ${String(error?.message || error)} Nenhuma exclusão foi mantida.`, "error");
    }
  }
  ...
  function markInstalled() {
    ...
    if (button) {
      ...
      button.title = "Consolidação V303 com recomendações ampliadas e verificação pós-gravação.";
    }
    return Boolean(root && button);
  }

  function install() {
    if (globalThis.__aldusDuplicateBatchAuthoritativeV303) return;
    ...
    globalThis.__aldusDuplicateBatchAuthoritativeV303 = Object.freeze({
      version: VERSION,
