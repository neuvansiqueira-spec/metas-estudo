...
/* V372: estabilidade da conclusão da Fábrica sem custo contínuo de renderização. */
(() => {
  "use strict";
  const VERSION = "20260822-factory-completion-stability-v372";
  const FLAG = "__ALDUS_FACTORY_COMPLETION_STABILITY_V372__";
  if (globalThis[FLAG]) return;

  const isDriveFolder = (value) => /drive\.google\.com\/drive\/(?:u\/\d+\/)?folders\//i.test(String(value || ""));

  function protectDestinationFolderSemantics() {
    const original = globalThis.factoryResumoAulaFolderMaterialLink;
    if (typeof original !== "function" || original.__aldusFactoryV372) return false;
    const wrapped = function factoryResumoAulaFolderMaterialLinkV372(...args) {
      const value = original.apply(this, args);
      return isDriveFolder(value) ? "" : value;
    };
    Object.defineProperty(wrapped, "__aldusFactoryV372", { value: true });
    Object.defineProperty(wrapped, "__aldusOriginal", { value: original });
    globalThis.factoryResumoAulaFolderMaterialLink = wrapped;
    return true;
  }

  function removeRepeatedDestinationTriggers() {
    ["syncFactoryWithActiveEdital", "checkCloudForUpdatesAfterAuth"].forEach((name) => {
      const current = globalThis[name];
      if (typeof current !== "function" || !current.__destinationV237 || typeof current.__aldusOriginal !== "function") return;
      globalThis[name] = current.__aldusOriginal;
    });
  }

  function install() {
    const protectedFolder = protectDestinationFolderSemantics();
    removeRepeatedDestinationTriggers();
    if (!protectedFolder) return false;
