(() => {
  ...
  const PATCH_VERSION = "20260727-inicializacao-unica-segura-v167";
  const CHECK_INTERVAL_MS = 60_000;
  const BANNER_ID = "aldusUpdateBannerV167";
  let lastCheckAt = 0;
  let registrationRef = null;
  let hadController = Boolean(navigator.serviceWorker?.controller);
  const watchedRegistrations = new WeakSet();

  if (typeof document === "undefined" || !("serviceWorker" in navigator) || globalThis.__ALDUS_UPDATE_FLOW_V167__) return;

  function injectStyles() {
    if (document.getElementById("aldusUpdateStylesV167")) return;
    const style = document.createElement("style");
    style.id = "aldusUpdateStylesV167";
    style.textContent = `
      #${BANNER_ID} {
        position: fixed;
        right: 18px;
      ...
      #${BANNER_ID} .aldus-update-copy-v167 { display: grid; gap: 3px; min-width: 0; }
  ...
  function ensureBanner() {
    ...
    banner.innerHTML = `
      <span class="aldus-update-copy-v167">
      ...
      <button type="button" data-aldus-update-now-v167>Atualizar agora</button>
    `;
    banner.querySelector("[data-aldus-update-now-v167]")?.addEventListener("click", () => {
      try { sessionStorage.setItem("aldus.update.reload.v167", PATCH_VERSION); } catch {}
      location.reload();
    });
  ...
  async function checkForUpdate(force = false) {
    ...
    try {
      ...
      console.warn("[Aldus v167] Não foi possível verificar atualização agora.", error);
      return false;
    }
  }
  ...
  function install() {
    ...
    Object.defineProperty(globalThis, "__ALDUS_UPDATE_FLOW_V167__", {
      value: Object.freeze({
